...
export default function LateFeeSetting() {
  ...
  useEffect(() => {
    const fetchDropdowns = async () => {
      try {
        ...
        const [gRes, ftRes, instRes] = await Promise.all([
  ...
  const fetchSettings = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${API_URL}/api/fee-master-settings/LATE_FEE_SETTINGS`, {
  ...
  const handleSave = async () => {
    if (!selectedFeeType || !dueDate) {
      showToast('Please select Fee Type and Due Date', 'error');
      return;
    }
    ...
    try {
      ...
      const existingIndex = settingsList.findIndex(
        s => s.groupId === selectedGroup && s.feeTypeId === selectedFeeType && s.installmentId === selectedInstallment
      );
      ...
      if (res.ok) {
        showToast('Late Fee Setting Saved');
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #e2e8f0', fontSize: '13px' }}>
            ...
            <tbody>
              {settingsList.length === 0 ? (
                <tr>
                  <td colSpan="7" style={{ textAlign: 'center', padding: '20px', color: '#64748b' }}>No Late Fee Settings Found</td>
                </tr>
              ) : (
