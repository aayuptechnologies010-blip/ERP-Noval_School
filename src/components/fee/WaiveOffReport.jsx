...
export default function WaiveOffReport() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        let filtered = data.filter(r => {
          ...
          const isDateInRange = rDate >= start && rDate <= end;
          // Assume Waive Offs are recorded as 'Adjustment' or specifically 'Waive Off'
          const isWaiveOff = r.paymentMode === 'Adjustment' || r.paymentMode === 'Waive Off' || r.transactionType === 'Waive Off';
          return isDateInRange && isWaiveOff && r.status === 'Successful';
        });
        ...
        setError(data.message || 'Failed to fetch waive off data');
      }
    } catch (err) {
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Fee Waive Off Report</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Select criteria and click "Show" to view manual waive-offs and adjustments.
            </div>
          ) : (
            <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', whiteSpace: 'nowrap' }}>
                  <thead style={{ background: '#f9fafb' }}>
                    <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <th style={{ padding: '12px 16px', textAlign: 'left', color: '#374151' }}>Receipt/Adj No</th>
                      ...
                      <th style={{ padding: '12px 16px', textAlign: 'left', color: '#374151' }}>Type</th>
                      <th style={{ padding: '12px 16px', textAlign: 'right', color: '#374151' }}>Waive Off Amount (₹)</th>
                  ...
                  <tbody>
                    {reportData.map((r, i) => (
                      <tr key={r._id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                        ...
                        <td style={{ padding: '12px 16px', color: '#4b5563' }}>{r.transactionType || r.paymentMode}</td>
                        <td style={{ padding: '12px 16px', color: '#059669', textAlign: 'right', fontWeight: 'bold' }}>
                          {Math.abs(r.amountPaid || r.discountAmount || 0)}
                        </td>
                  ...
                  {reportData.length > 0 && (
                    <tfoot style={{ background: '#ecfdf5' }}>
                      <tr>
                        <td colSpan="5" style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#065f46' }}>Total Amount Waived:</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#059669', fontSize: '15px' }}>
                          ₹{reportData.reduce((acc, curr) => acc + Math.abs(curr.amountPaid || curr.discountAmount || 0), 0).toLocaleString()}
                        </td>
                        <td></td>
                      </tr>
                    </tfoot>
