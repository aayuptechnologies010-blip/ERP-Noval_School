...
export default function ManualFeeReport() {
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Manual fee = receipts with paymentMode "Manual" or manually entered (no online flag)
        let filtered = data.filter(r => {
          const rDate = new Date(r.receiptDate || r.createdAt);
          const inRange = rDate >= start && rDate <= end;
          const isManual = !r.isOnline && r.status === 'Successful';
          return inRange && isManual;
        });
        ...
        if (searchStudent.trim()) {
          ...
          filtered = filtered.filter(r =>
            ...
            r.student?.admissionNumber?.includes(q)
          );
        }
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Manual Fee Report</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Select filters and click "Show" to view manually collected fee receipts.
            </div>
          ) : (
            <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', whiteSpace: 'nowrap' }}>
                  ...
                  {reportData.length > 0 && (
                    <tfoot style={{ background: '#ecfdf5' }}>
                      <tr>
                        <td colSpan="6" style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#065f46' }}>Total Manual Collection:</td>
