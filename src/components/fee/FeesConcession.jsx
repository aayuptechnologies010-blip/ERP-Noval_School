...
export default function FeesConcession() {
  ...
  const handleShow = async () => {
    ...
    setError(null);
    try {
      // For concession report, we fetch all receipts and filter those with discountAmount > 0
      const res = await fetch(`${API_URL}/api/fee-transactions/receipts`, { headers: { Authorization: `Bearer ${token}` } });
      const data = await res.json();
      
      if (res.ok) {
        let filtered = data.filter(r => r.status === 'Successful' && r.discountAmount && r.discountAmount > 0);
        ...
        setError(data.message || 'Failed to fetch concession data');
      }
    } catch (err) {
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Fee Concession Report</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Click "Show" to view all concessions applied.
            </div>
          ) : (
            <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', whiteSpace: 'nowrap' }}>
                  <thead style={{ background: '#f9fafb' }}>
                    <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <th style={{ padding: '12px 16px', textAlign: 'right', color: '#374151' }}>Amount Paid (₹)</th>
                      <th style={{ padding: '12px 16px', textAlign: 'right', color: '#059669' }}>Concession (₹)</th>
                  ...
                  <tbody>
                    {reportData.map((r, i) => (
                      <tr key={r._id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                        ...
                        <td style={{ padding: '12px 16px', color: '#4b5563', textAlign: 'right' }}>{r.amountPaid}</td>
                        <td style={{ padding: '12px 16px', color: '#059669', textAlign: 'right', fontWeight: 'bold' }}>{r.discountAmount}</td>
                  ...
                  {reportData.length > 0 && (
                    <tfoot style={{ background: '#ecfdf5' }}>
                      <tr>
                        <td colSpan="5" style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#065f46' }}>Total Concessions Provided:</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#059669', fontSize: '15px' }}>
                          ₹{reportData.reduce((acc, curr) => acc + curr.discountAmount, 0).toLocaleString()}
                        </td>
                        <td></td>
                      </tr>
                    </tfoot>
