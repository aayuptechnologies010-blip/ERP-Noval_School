...
export default function ReceiptWiseDailyCollection() {
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Flatten the receipts to a single list
        const flattenedReceipts = data.flatMap(group => 
        ...
        setReportData(flattenedReceipts);
      } else {
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Receipt Wise Daily Collection</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Select a date range and click "Show" to view the receipt list.
            </div>
          ) : (
            <div>
                  ...
                  <tbody>
                    {reportData.map((r, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #e5e7eb' }}>
                        <td style={{ padding: '12px 16px', color: '#4b5563' }}>{r.transactionDate}</td>
                        <td style={{ padding: '12px 16px', color: '#1f2937', fontWeight: '500' }}>{r.receiptNo}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563' }}>{r.admissionNo}</td>
                        <td style={{ padding: '12px 16px', color: '#1f2937' }}>{r.studentName}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563' }}>{r.class}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563' }}>{r.payMode}</td>
              ...
              {reportData.length > 0 && (
                <div style={{ marginTop: '20px', padding: '16px', background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#059669' }}>
                    ₹{reportData.reduce((acc, curr) => acc + curr.amountPaid, 0)}
                  </div>
                </div>
              )}
            </div>
