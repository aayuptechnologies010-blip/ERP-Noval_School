  ...
  const handleShow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Filter specifically for Bounced cheques
        let filtered = data.filter(r => {
          const rDate = new Date(r.receiptDate || r.createdAt);
          return r.chequeStatus === 'Bounced' && rDate >= start && rDate <= end;
        });
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Bounced Cheque Report</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Select a date range and click "Show" to view bounced cheques.
            </div>
          ) : (
            <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', whiteSpace: 'nowrap' }}>
                  <thead style={{ background: '#f9fafb' }}>
                    <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <th style={{ padding: '12px 16px', textAlign: 'left', color: '#991b1b' }}>Bounce Reason</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reportData.map((r, i) => (
                      <tr key={r._id} style={{ borderBottom: '1px solid #e5e7eb', background: '#fef2f2' }}>
                        ...
                        <td style={{ padding: '12px 16px', color: '#1f2937', fontWeight: 'bold' }}>{r.referenceNumber || '-'}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563' }}>{r.bankName || '-'}</td>
                        <td style={{ padding: '12px 16px', color: '#b91c1c', textAlign: 'right', fontWeight: 'bold' }}>{r.amountPaid}</td>
                        <td style={{ padding: '12px 16px', color: '#b91c1c' }}>{r.cancelledReason || 'Insufficient Funds'}</td>
                      </tr>
                    ))}
                  </tbody>
                  {reportData.length > 0 && (
                    <tfoot style={{ background: '#fecaca' }}>
                      <tr>
                        <td colSpan="6" style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#991b1b' }}>Total Bounced Value:</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#991b1b', fontSize: '15px' }}>
