...
export default function UnpaidStudentReport() {
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.message || 'Failed to fetch unpaid students');
      }
    } catch (err) {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', background: '#f3f4f6' }}>
      ...
      <div style={{ width: '250px', background: '#fff', borderRight: '1px solid #e5e7eb', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px', flexShrink: 0, overflowY: 'auto' }}>
        ...
        <div>
          <label style={labelStyle}>Class Selection</label>
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Unpaid Student Consolidated Report</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Click "Show" to generate the unpaid student report.
            </div>
          ) : (
            <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                  <thead style={{ background: '#f9fafb' }}>
                    <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <th style={{ padding: '12px 16px', textAlign: 'right', color: '#991b1b' }}>Total Unpaid (₹)</th>
                    </tr>
                  </thead>
                  <tbody>
                  ...
                  {reportData.length > 0 && (
                    <tfoot style={{ background: '#fef2f2' }}>
                      <tr>
                        <td colSpan="4" style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#991b1b' }}>Grand Total Outstanding:</td>
