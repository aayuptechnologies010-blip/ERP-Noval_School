...
export default function TransportStudentList() {
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        let students = (Array.isArray(data) ? data : (data.students || [])).filter(s => s.transportAssigned || s.transportRoute);
        if (selectedClass) {
          const cls = classes.find(c => c._id === selectedClass)?.name;
          students = students.filter(s => (s.class?.name || s.class) === cls);
        }
        if (selectedRoute) {
          students = students.filter(s => s.transportRoute?._id === selectedRoute || s.transportRoute === selectedRoute);
        }
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Transport Student List</h2>
            {reportData.length > 0 && (
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                <span style={{ fontSize: '13px', color: '#6b7280' }}>Total: <strong>{reportData.length}</strong></span>
          ...
          {reportData.length === 0 && !loading ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280' }}>Click "Show" to view transport students list.</div>
          ) : (
            <div style={{ overflowX: 'auto', border: '1px solid #e5e7eb', borderRadius: '8px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', whiteSpace: 'nowrap' }}>
                <thead style={{ background: '#f9fafb' }}>
                  <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                    {['Sr.','Adm No','Student Name','Class','Route','Stop','Monthly Fee (₹)'].map((h, i) => (
