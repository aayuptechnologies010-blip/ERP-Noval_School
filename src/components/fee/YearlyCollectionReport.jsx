...
export default function YearlyCollectionReport() {
  ...
  const handleShow = async () => {
    ...
    setError(null);
    try {
      let url = `${API_URL}/api/fee-reports/collection/monthly`; // Without year, it returns all years
      ...
      if (res.ok) {
        ...
        const groupedByYear = data.reduce((acc, curr) => {
          ...
          if (!acc[year]) {
            acc[year] = { year, totalAmount: 0, count: 0 };
          }
          acc[year].totalAmount += curr.totalAmount;
          acc[year].count += curr.count;
          return acc;
        }, {});
        
        setReportData(Object.values(groupedByYear).sort((a, b) => b.year - a.year));
      } else {
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Yearly Collection Report</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Click "Show" to view the year-wise collection report.
            </div>
          ) : (
            <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                  <thead style={{ background: '#f3f4f6' }}>
                    <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                      <th style={{ padding: '12px 16px', textAlign: 'left', color: '#374151' }}>Year</th>
                      <th style={{ padding: '12px 16px', textAlign: 'center', color: '#374151' }}>Total Transactions</th>
                  ...
                  <tbody>
                    {reportData.map((row, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #e5e7eb' }}>
                        <td style={{ padding: '12px 16px', color: '#1f2937', fontWeight: '500', fontSize: '15px' }}>{row.year}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563', textAlign: 'center' }}>{row.count}</td>
                        <td style={{ padding: '12px 16px', color: '#059669', textAlign: 'right', fontWeight: 'bold', fontSize: '15px' }}>{row.totalAmount.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {reportData.length > 0 && (
                <div style={{ marginTop: '20px', padding: '16px', background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#065f46' }}>Lifetime Collection</div>
