...
export default function DateConsolidatedCollectionReportPaymodeWise() {
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Pivot data: Group by Date, then separate by Paymode
        const groupedByDate = data.reduce((acc, curr) => {
          const date = curr._id.date;
          const mode = curr._id.paymentMode || 'Unknown';
          
          if (!acc[date]) {
            acc[date] = { date, Cash: 0, Online: 0, Cheque: 0, DD: 0, Card: 0, Adjustment: 0, Total: 0 };
          }
          
          if (acc[date][mode] !== undefined) {
            acc[date][mode] += curr.totalAmount;
          } else {
            acc[date][mode] = curr.totalAmount; // fallback for custom modes
          }
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Consolidated Date & Paymode Wise</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Select a date range and click "Show" to view the paymode-wise consolidated table.
            </div>
          ) : (
            <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                  <thead style={{ background: '#f9fafb' }}>
                    <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <th style={{ padding: '12px 16px', textAlign: 'right', color: '#374151', background: '#ecfdf5' }}>Day Total (₹)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reportData.map((row, i) => (
                      <tr key={i} style={{ borderBottom: '1px solid #e5e7eb' }}>
                        ...
                        <td style={{ padding: '12px 16px', color: '#4b5563', textAlign: 'right' }}>{row.Cash || '-'}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563', textAlign: 'right' }}>{row.Online || '-'}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563', textAlign: 'right' }}>{row.Cheque || '-'}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563', textAlign: 'right' }}>{row.DD || '-'}</td>
                        <td style={{ padding: '12px 16px', color: '#4b5563', textAlign: 'right' }}>{row.Card || '-'}</td>
                        <td style={{ padding: '12px 16px', color: '#059669', textAlign: 'right', fontWeight: 'bold', background: '#ecfdf5' }}>{row.Total}</td>
                      </tr>
                    ))}
                  </tbody>
                  {reportData.length > 0 && (
                    <tfoot style={{ background: '#f3f4f6' }}>
                      <tr>
                        <td style={{ padding: '12px 16px', fontWeight: 'bold', color: '#1f2937' }}>Grand Totals</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#1f2937' }}>{reportData.reduce((acc, curr) => acc + (curr.Cash || 0), 0)}</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#1f2937' }}>{reportData.reduce((acc, curr) => acc + (curr.Online || 0), 0)}</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#1f2937' }}>{reportData.reduce((acc, curr) => acc + (curr.Cheque || 0), 0)}</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#1f2937' }}>{reportData.reduce((acc, curr) => acc + (curr.DD || 0), 0)}</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#1f2937' }}>{reportData.reduce((acc, curr) => acc + (curr.Card || 0), 0)}</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#059669', background: '#d1fae5', fontSize: '14px' }}>
