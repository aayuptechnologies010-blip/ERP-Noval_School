...
export default function AdvancePaymentReport() {
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Filter students who have an advance amount > 0
        let filtered = data.filter(s => s.advanceAmount > 0);
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Advance Payment Report</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Click "Show" to view students holding advance fee balances.
            </div>
          ) : (
            <div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', whiteSpace: 'nowrap' }}>
                  <thead style={{ background: '#f9fafb' }}>
                    <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <th style={{ padding: '12px 16px', textAlign: 'left', color: '#374151' }}>Mobile Number</th>
                      <th style={{ padding: '12px 16px', textAlign: 'right', color: '#2563eb' }}>Advance Balance (₹)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reportData.map((s, i) => (
                      <tr key={s.admissionNo} style={{ borderBottom: '1px solid #e5e7eb' }}>
                        ...
                        <td style={{ padding: '12px 16px', color: '#4b5563' }}>{s.mobileNumber}</td>
                        <td style={{ padding: '12px 16px', color: '#2563eb', textAlign: 'right', fontWeight: 'bold' }}>{s.advanceAmount}</td>
                      </tr>
                    ))}
                  </tbody>
                  {reportData.length > 0 && (
                    <tfoot style={{ background: '#eff6ff' }}>
                      <tr>
                        <td colSpan="5" style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#1d4ed8' }}>Total Advance Held:</td>
                        <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: '#2563eb', fontSize: '15px' }}>
