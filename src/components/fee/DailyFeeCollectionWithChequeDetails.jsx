...
export default function DailyFeeCollectionWithChequeDetails() {
  ...
  const handleShow = async () => {
    ...
    setError(null);
    try {
      // Force payMode=Cheque
      let url = `${API_URL}/api/fee-reports/collection/daily?startDate=${startDate}&endDate=${endDate}&payMode=Cheque`;
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Daily Collection With Cheque Details</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Select a date range and click "Show" to view the Cheque collections.
            </div>
          ) : (
            <div>
              {reportData.map((group, index) => (
                <div key={index} style={{ marginBottom: '30px' }}>
                  <div style={{ background: '#f9fafb', padding: '12px 16px', border: '1px solid #e5e7eb', borderBottom: 'none', borderRadius: '8px 8px 0 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ fontSize: '13px', color: '#4b5563' }}>Total Cheques: <b>{group.count}</b> | Total Amount: <b style={{ color: '#059669' }}>₹{group.totalAmount}</b></div>
                  </div>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                      <thead style={{ background: '#f3f4f6' }}>
                        <tr>
                          ...
                          <th style={{ padding: '10px 16px', textAlign: 'left', color: '#374151' }}>Bank Name</th>
                          <th style={{ padding: '10px 16px', textAlign: 'left', color: '#374151' }}>Cheque No</th>
                          <th style={{ padding: '10px 16px', textAlign: 'left', color: '#374151' }}>Status</th>
                      ...
                      <tbody>
                        {group.receipts.map((r, i) => (
                          <tr key={i} style={{ borderTop: '1px solid #e5e7eb' }}>
                            ...
                            <td style={{ padding: '10px 16px', color: '#4b5563' }}>{r.bankName || 'N/A'}</td>
                            <td style={{ padding: '10px 16px', color: '#4b5563' }}>{r.referenceNumber || 'N/A'}</td>
                            <td style={{ padding: '10px 16px', color: '#4b5563' }}>
                              <span style={{ padding: '2px 8px', borderRadius: '12px', fontSize: '11px', fontWeight: 'bold', background: r.chequeStatus === 'Cleared' ? '#d1fae5' : (r.chequeStatus === 'Bounced' ? '#fee2e2' : '#fef3c7'), color: r.chequeStatus === 'Cleared' ? '#065f46' : (r.chequeStatus === 'Bounced' ? '#991b1b' : '#92400e') }}>
              ...
              {reportData.length > 0 && (
                <div style={{ marginTop: '20px', padding: '16px', background: '#ecfdf5', border: '1px solid #a7f3d0', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ fontSize: '16px', fontWeight: 'bold', color: '#065f46' }}>Total Cheque Collection</div>
