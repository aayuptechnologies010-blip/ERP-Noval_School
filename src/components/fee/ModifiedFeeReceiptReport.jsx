...
export default function ModifiedFeeReceiptReport() {
  ...
  const handleShow = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Modified receipts: those with isModified flag or having modifiedAt field
        const filtered = data.filter(r => {
          const rDate = new Date(r.modifiedAt || r.updatedAt || r.createdAt);
          const inRange = rDate >= start && rDate <= end;
          const isModified = r.isModified || (r.modifiedAt && r.modifiedAt !== r.createdAt);
          return inRange && isModified;
        });
      ...
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
        <div style={{ background: '#fff', width: '100%', minHeight: '100%', borderRadius: '8px', border: '1px solid #e5e7eb', padding: '20px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #e5e7eb', paddingBottom: '16px', marginBottom: '20px' }}>
            <h2 style={{ margin: 0, fontSize: '18px', color: '#1f2937' }}>Modified Fee Receipt Report</h2>
            {reportData.length > 0 && (
          ...
          {reportData.length === 0 && !loading && !error ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', fontSize: '14px' }}>
              Select a date range and click "Show" to view modified/corrected receipts.
            </div>
          ) : (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', whiteSpace: 'nowrap' }}>
                <thead style={{ background: '#f9fafb' }}>
                  <tr style={{ borderBottom: '1px solid #e5e7eb' }}>
                    <th style={{ padding: '12px 16px', textAlign: 'left', color: '#374151' }}>Original Date</th>
                    <th style={{ padding: '12px 16px', textAlign: 'left', color: '#374151' }}>Modified Date</th>
                ...
                <tbody>
                  {reportData.map((r, i) => (
                    <tr key={r._id} style={{ borderBottom: '1px solid #e5e7eb', background: '#fffbeb' }}>
                      <td style={{ padding: '12px 16px', color: '#4b5563' }}>{new Date(r.createdAt).toLocaleDateString()}</td>
                      <td style={{ padding: '12px 16px', color: '#d97706', fontWeight: 'bold' }}>{new Date(r.modifiedAt || r.updatedAt).toLocaleDateString()}</td>
                      <td style={{ padding: '12px 16px', color: '#1f2937', fontWeight: '600' }}>{r.receiptNo}</td>
                      <td style={{ padding: '12px 16px', color: '#4b5563' }}>{r.student?.firstName} {r.student?.lastName}</td>
                      <td style={{ padding: '12px 16px', color: '#4b5563' }}>{r.student?.class}</td>
                      <td style={{ padding: '12px 16px', color: '#059669', textAlign: 'right', fontWeight: 'bold' }}>{r.amountPaid}</td>
