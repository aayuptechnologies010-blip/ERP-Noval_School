...
import {
  ...
  AlertCircle, Sparkles, Calendar, Users, Landmark,
  ChevronLeft, ChevronRight, FileCheck, IndianRupee,
...
export default function PensionListReport() {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/salary-structure/pension-list-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch Pension List report');
      }
    } catch (err) {
      console.error(err);
  ...
  const filteredRecords = records.filter(r => {
    ...
    return (
      ...
      (r.ppoNumber && r.ppoNumber.toLowerCase().includes(s)) ||
  ...
  const exportCSV = () => {
    ...
    const headers = [
      ...
      'PPO Number', 'Commencement Date', 'Basic Pension', '40% Commuted',
      'Reduced Pension', '50% Dearness Relief', 'Net Monthly Pension',
      'Disbursement Bank', 'Bank A/c No', 'Status'
    ];
    ...
    link.setAttribute('download', `Ayup_Tech_Pension_List_Report.csv`);
      ...
      <div style={{
        ...
        <div style={{ padding: '20px', borderBottom: '1px solid #e2e8f0', background: 'linear-gradient(135deg, #0f172a, #1e293b)', color: 'white' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600, letterSpacing: '0.3px' }}>Pension Registry</h3>
          </div>
          <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#94a3b8' }}>Ayup Tech PPO & Disbursement Filter</p>
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <h1 style={{ margin: '4px 0 0 0', fontSize: '20px', fontWeight: 700, color: '#0f172a' }}>
              Pensioners Master Register & Monthly Payroll
            </h1>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            >
              <Printer size={15} /> Print PPO Register
            </button>
          </div>
        </div>
        ...
        <div style={{ padding: '16px 24px 0 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Active Pensioners</span>
            ...
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#0f172a', marginTop: '6px' }}>
              {summary.totalPensioners ?? records.length}
            </div>
            <div style={{ fontSize: '11px', color: '#0284c7', marginTop: '2px', fontWeight: 500 }}>
              Ayup Tech Pensioners
            </div>
          </div>

          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Total Basic Pension</span>
              <div style={{ padding: '6px', borderRadius: '8px', backgroundColor: '#fef3c7', color: '#d97706' }}><Banknote size={16} /></div>
            </div>
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#d97706', marginTop: '6px' }}>
              ₹{(summary.totalBasicPension || 0).toLocaleString('en-IN')}
            </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Dearness Relief (DR)</span>
              <div style={{ padding: '6px', borderRadius: '8px', backgroundColor: '#f3e8ff', color: '#7c3aed' }}><IndianRupee size={16} /></div>
            </div>
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#7c3aed', marginTop: '6px' }}>
              ₹{(summary.totalDearnessRelief || 0).toLocaleString('en-IN')}
            </div>
            <div style={{ fontSize: '11px', color: '#7c3aed', marginTop: '2px', fontWeight: 500 }}>
              {summary.drRate || '50% Dearness Relief'}
            </div>
          </div>

          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Net Monthly Outflow</span>
            ...
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#16a34a', marginTop: '6px' }}>
              ₹{(summary.totalNetMonthlyPension || 0).toLocaleString('en-IN')}
            </div>
            <div style={{ fontSize: '11px', color: '#16a34a', marginTop: '2px', fontWeight: 500 }}>
              Avg: ₹{(summary.averageMonthlyPension || 0).toLocaleString('en-IN')} / Pensioner
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
          <div style={{ position: 'relative', width: '320px' }}>
            ...
            <input
              type="text"
              placeholder="Search staff, PPO number, bank a/c..."
              value={searchTerm}
          ...
          <div style={{ fontSize: '12px', color: '#64748b', fontWeight: 500 }}>
            Showing <b>{filteredRecords.length}</b> pensioners
          </div>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '12px' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#475569', fontWeight: 600 }}>
                  ...
                  <th style={{ padding: '12px 14px' }}>Pensioner Name</th>
                  <th style={{ padding: '12px 14px' }}>PPO Number</th>
                  <th style={{ padding: '12px 14px' }}>Commenced</th>
                  <th style={{ padding: '12px 14px', textAlign: 'right' }}>Basic Pension</th>
                  <th style={{ padding: '12px 14px', textAlign: 'right' }}>Commuted (40%)</th>
                  <th style={{ padding: '12px 14px', textAlign: 'right' }}>Reduced Pension</th>
                  <th style={{ padding: '12px 14px', textAlign: 'right' }}>DR (50%)</th>
                  <th style={{ padding: '12px 14px', textAlign: 'right' }}>Net Monthly</th>
                  <th style={{ padding: '12px 14px' }}>Bank Details</th>
                ...
                {filteredRecords.length > 0 ? (
                  filteredRecords.map((r, idx) => (
                    ...
                    >
                      ...
                      <td style={{ padding: '11px 14px', fontWeight: 600, color: '#0f172a' }}>
                        <div>{r.staffName}</div>
                        <div style={{ fontSize: '11px', color: '#64748b', fontWeight: 400 }}>{r.department} • {r.designation}</div>
                      </td>
                      <td style={{ padding: '11px 14px', fontFamily: 'monospace', fontWeight: 600, color: '#4338ca' }}>
                        {r.ppoNumber}
                      </td>
                      <td style={{ padding: '11px 14px', color: '#64748b' }}>{r.commencementDate}</td>
                      <td style={{ padding: '11px 14px', textAlign: 'right', fontWeight: 600, color: '#0f172a' }}>
                        ₹{(r.basicPension || 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '11px 14px', textAlign: 'right', color: '#b45309' }}>
                        ₹{(r.commutedPortion || 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '11px 14px', textAlign: 'right', color: '#475569' }}>
                        ₹{(r.reducedPension || 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '11px 14px', textAlign: 'right', color: '#7c3aed', fontWeight: 500 }}>
                        +₹{(r.dearnessRelief || 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '11px 14px', textAlign: 'right', fontWeight: 700, color: '#16a34a' }}>
                        ₹{(r.netMonthlyPension || 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '11px 14px' }}>
                        <div style={{ fontWeight: 500, color: '#1e293b' }}>{r.bankName}</div>
                        <div style={{ fontSize: '11px', color: '#64748b', fontFamily: 'monospace' }}>A/c: {r.bankAccountNo}</div>
                      </td>
                      <td style={{ padding: '11px 14px' }}>
                        <span style={{
                          display: 'inline-flex',
                          ...
                          <CheckCircle size={11} /> {r.disbursementStatus || 'Disbursed'}
                        </span>
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan={12} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                      {loading ? 'Loading pension list records...' : 'No pension records found.'}
                    </td>
                  </tr>
                )}
              </tbody>
