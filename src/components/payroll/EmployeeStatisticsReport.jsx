...
import {
  ...
  AlertCircle, Sparkles, Calendar, Users, BarChart2,
  ChevronLeft, ChevronRight, Filter, PieChart, IndianRupee,
...
export default function EmployeeStatisticsReport() {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/salary-structure/employee-statistics-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch Employee Statistics report');
      }
    } catch (err) {
      console.error(err);
  ...
  const exportCSV = () => {
    ...
    const csvHeaders = [
      'Sr No', 'Emp ID', 'Staff Name', 'Gender', 'Department',
      'Designation', 'Staff Type', 'Basic Salary', 'Gross Salary',
      'Net Salary', 'Grade Pay', 'Mobile No', 'Status'
    ];
    ...
    link.setAttribute('download', `Ayup_Tech_Employee_Statistics_${monthYear}.csv`);
      ...
      <div style={{
        ...
        <div style={{ padding: '20px', borderBottom: '1px solid #e2e8f0', background: 'linear-gradient(135deg, #0f172a, #1e293b)', color: 'white' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600, letterSpacing: '0.3px' }}>Demographics Filter</h3>
          </div>
          <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#94a3b8' }}>Ayup Tech Workforce Analytics</p>
        </div>
        ...
        <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', flex: 1 }}>
          ...
          <div>
            ...
            >
              {filterOptions.months?.length ? filterOptions.months.map(m => (
          ...
          <div>
            <label style={{ fontSize: '12px', fontWeight: 600, color: '#475569', marginBottom: '6px', display: 'block' }}>Group By Categorization</label>
            <select
              value={groupBy}
            ...
            >
              ...
              <option value="Staff Type">Staff Type Wise</option>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#16a34a', fontWeight: 600, background: '#dcfce7', padding: '2px 8px', borderRadius: '4px' }}>
                <Sparkles size={12} /> Ayup Tech Staff
              </span>
              <span style={{ fontSize: '11px', color: '#64748b', background: '#f1f5f9', padding: '2px 8px', borderRadius: '4px', fontWeight: 500 }}>
                {monthYear}
              </span>
            </div>
            <h1 style={{ margin: '4px 0 0 0', fontSize: '20px', fontWeight: 700, color: '#0f172a' }}>
              Employee Workforce Statistics & Demographics
            </h1>
          </div>
        ...
        <div style={{ padding: '16px 24px 0 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Total Workforce</span>
            ...
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#0f172a', marginTop: '6px' }}>
              {summary.totalStaff ?? records.length}
            </div>
            <div style={{ fontSize: '11px', color: '#0284c7', marginTop: '2px', fontWeight: 500 }}>
              Ayup Tech Verified Active Staff
            </div>
          </div>

          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Gender Ratio</span>
              <div style={{ padding: '6px', borderRadius: '8px', backgroundColor: '#fce7f3', color: '#db2777' }}><PieChart size={16} /></div>
            </div>
            <div style={{ fontSize: '20px', fontWeight: 700, color: '#db2777', marginTop: '6px' }}>
              {summary.genderRatioText || `${summary.maleCount || 0} M / ${summary.femaleCount || 0} F`}
            </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Teaching vs Non-Teaching</span>
              <div style={{ padding: '6px', borderRadius: '8px', backgroundColor: '#dcfce7', color: '#16a34a' }}><Briefcase size={16} /></div>
            </div>
            <div style={{ fontSize: '20px', fontWeight: 700, color: '#16a34a', marginTop: '6px' }}>
              {summary.teachingCount || 0} T / {summary.nonTeachingCount || 0} NT
            </div>
            <div style={{ fontSize: '11px', color: '#16a34a', marginTop: '2px', fontWeight: 500 }}>
              Faculty & Staff Balance
            </div>
          </div>

          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Avg Monthly Pay</span>
            ...
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#d97706', marginTop: '6px' }}>
              ₹{(summary.avgGross || 0).toLocaleString('en-IN')}
            </div>
            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>
              Total Gross: ₹{(summary.totalGross || 0).toLocaleString('en-IN')}
            </div>
          </div>
        </div>
        ...
        {groupBreakdown.length > 0 && (
          <div style={{ padding: '14px 24px 0 24px' }}>
            <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '14px 18px' }}>
              <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '10px', textTransform: 'uppercase', letterSpacing: '0.4px' }}>
                Distribution by {groupBy}
              </div>
              <div style={{ display: 'flex', gap: '12px', overflowX: 'auto', paddingBottom: '4px' }}>
                {groupBreakdown.map((g, i) => (
                  <div key={i} style={{ minWidth: '180px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '10px 14px' }}>
                    <div style={{ fontSize: '12px', fontWeight: 700, color: '#0f172a' }}>{g.name}</div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px', fontSize: '11px', color: '#64748b' }}>
                      <span>Headcount: <b>{g.count}</b></span>
                      <span>Avg: <b>₹{(g.avgGross || 0).toLocaleString('en-IN')}</b></span>
                    </div>
                    <div style={{ width: '100%', height: '4px', backgroundColor: '#e2e8f0', borderRadius: '2px', marginTop: '6px', overflow: 'hidden' }}>
                      <div style={{ width: `${Math.min(100, (g.count / (records.length || 1)) * 100)}%`, height: '100%', backgroundColor: '#0284c7' }} />
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div style={{ padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
          ...
          <div style={{ fontSize: '12px', color: '#64748b', fontWeight: 500 }}>
            Showing <b>{filteredRecords.length}</b> staff profiles
          </div>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '12px' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#475569', fontWeight: 600 }}>
                  ...
                  <th style={{ padding: '12px 14px' }}>Gender</th>
                ...
                {filteredRecords.length > 0 ? (
                    ...
                    >
                      ...
                      <td style={{ padding: '11px 14px' }}>
                        <span style={{
                          ...
                          fontSize: '11px',
                          fontWeight: 600,
                          backgroundColor: r.gender === 'Female' ? '#fce7f3' : '#e0f2fe',
                          color: r.gender === 'Female' ? '#db2777' : '#0369a1'
                        }}>
                          {r.gender}
                        </span>
                      </td>
                      ...
                      <td style={{ padding: '11px 14px' }}>
                        <span style={{
                          ...
                          fontSize: '11px',
                          fontWeight: 500,
                          backgroundColor: r.staffType === 'Teaching' ? '#dcfce7' : '#f1f5f9',
                          color: r.staffType === 'Teaching' ? '#15803d' : '#475569'
                        }}>
                          {r.staffType}
                        </span>
                      </td>
                      ...
                      <td style={{ padding: '11px 14px', color: '#64748b', fontSize: '11px' }}>{r.mobile}</td>
                      <td style={{ padding: '11px 14px' }}>
                        <span style={{
                          display: 'inline-flex',
                          alignItems: 'center',
                          ...
                          <CheckCircle size={11} /> {r.status || 'Active'}
                        </span>
                      </td>
                    </tr>
                  ))
                  ...
                  <tr>
                    <td colSpan={12} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                      {loading ? 'Loading workforce demographics...' : 'No statistics records found.'}
                    </td>
                  </tr>
                )}
              </tbody>
