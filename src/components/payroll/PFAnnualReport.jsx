...
import {
  ...
  ChevronLeft, ChevronRight, Filter, Building2, Calendar
} from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';

export default function PFAnnualReport() {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/salary-structure/pf-annual-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch PF Annual report');
      }
    } catch (err) {
      console.error(err);
  ...
  const exportCSV = () => {
    ...
    const headersList = [
      '#', 'Emp ID', 'Staff Name', 'UAN Number', 'PF Number', 'Department', 'Months Count',
      'Annual Wages (₹)', 'Annual EE Share (₹)', 'Annual ER EPF (₹)', 'Annual ER EPS (₹)', 'Total Annual Remittance (₹)', 'FY'
    ];
    ...
    link.setAttribute('download', `PF_Annual_Form3A_6A_${financialYear}_AyupTech.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showNotif('success', 'PF Annual CSV downloaded successfully');
  };
        ...
        {isSidebarOpen && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', height: '100%' }}>
            ...
            <div style={{ marginTop: '10px' }}>
              ...
              >
                <Eye size={16} /> {loading ? 'Fetching...' : 'Show Annual Return'}
              </button>
            </div>
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowX: 'hidden' }}>
        ...
        <div style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '700', color: '#0f172a' }}>
                  PF Annual Statement (Form 3A / 6A)
                </h2>
                <span style={{
              ...
              <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                Consolidated Annual Return of Member Contributions across 12 Pay Periods (Apr to Mar)
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            >
              <Download size={14} /> Export Form 3A/6A
            </button>
        ...
        <div style={{ padding: '20px 24px 10px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#0284c7' }}>12-Month Coverage</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Annual EPF Wages</div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Annual Employee PF</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#16a34a' }}>₹{(summary.totalEeShare || 0).toLocaleString('en-IN')}</div>
              <div style={{ fontSize: '11px', color: '#16a34a' }}>A/c 01 Total Deductions</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Grand Annual PF Remittance</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#a21caf' }}>₹{(summary.grandTotalContribution || 0).toLocaleString('en-IN')}</div>
              <div style={{ fontSize: '11px', color: '#a21caf' }}>EE 12% + ER 12%</div>
            </div>
          </div>
        </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0', color: '#475569' }}>
                    ...
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right' }}>Annual EPF Wages (₹)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right', color: '#0369a1' }}>Annual EE Share (₹)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right', color: '#059669' }}>Annual ER EPF (₹)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right', color: '#d97706' }}>Annual ER EPS (₹)</th>
                  ...
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={12} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                        {loading ? 'Fetching Annual Form 3A/6A records...' : 'No records found.'}
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((r, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                        ...
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '600', color: '#0f172a' }}>₹{(r.annualEpfWages || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '600', color: '#0369a1' }}>₹{(r.annualEeShare || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '600', color: '#059669' }}>₹{(r.annualErEpfShare || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '600', color: '#d97706' }}>₹{(r.annualErEpsShare || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '700', color: '#7c3aed' }}>₹{(r.annualTotalContribution || 0).toLocaleString('en-IN')}</td>
                      </tr>
                    ))
                  )}
                </tbody>
                {filteredRecords.length > 0 && (
                  <tfoot>
                    <tr style={{ backgroundColor: '#f1f5f9', borderTop: '2px solid #cbd5e1', fontWeight: '700', color: '#0f172a' }}>
                      <td colSpan={7} style={{ padding: '12px 14px', textAlign: 'right' }}>Annual Totals:</td>
