...
import {
  ...
  AlertCircle, Sparkles, DollarSign, Users, TrendingUp,
...
export default function IncrementReport() {
  ...
  const fetchReport = async () => {
    try {
      ...
      if (monthYear && monthYear !== 'All' && monthYear !== 'All Months') params.append('monthYear', monthYear);
      ...
      if (status && status !== 'Select') params.append('status', status);
      if (searchTerm) params.append('search', searchTerm);

      const res = await fetch(`${API_BASE}/api/salary-structure/increment-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch Increment report');
      }
    } catch (err) {
      console.error(err);
  ...
  const filteredRecords = records.filter(r => {
    ...
    return (
      ...
      (r.orderNo && r.orderNo.toLowerCase().includes(s)) ||
  ...
  const exportCSV = () => {
    ...
    const headersList = [
      '#', 'Emp ID', 'Staff Name', 'Department', 'Designation',
      'Previous Basic (₹)', 'Increment Rate (%)', 'Increment Amount (₹)', 'Revised Basic (₹)', 'Order Number', 'Effective Date', 'Status', 'Month'
    ];
    ...
    link.setAttribute('download', `Annual_Increment_Report_${monthYear}_AyupTech.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showNotif('success', 'Increment Report CSV downloaded successfully');
  };
      ...
      <div style={{
        ...
        {isSidebarOpen && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', height: '100%' }}>
            ...
            <div className="form-group">
              <label style={{ fontSize: '12px', fontWeight: '600', color: '#4a5568', marginBottom: '6px', display: 'block' }}>Status</label>
              <select value={status} onChange={e => setStatus(e.target.value)} style={{ width: '100%', padding: '8px 10px', borderRadius: '6px', border: '1px solid #cbd5e0', fontSize: '12px' }}>
                <option value="Select">All Statuses</option>
                <option value="Approved & Implemented">Approved & Implemented</option>
            ...
            <div style={{ marginTop: '10px' }}>
              ...
              >
                <Eye size={16} /> {loading ? 'Fetching...' : 'Show Increment Ledger'}
              </button>
            </div>
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowX: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                Staff Annual 3% Basic Increment Sanction Ledger • Established by Ayup Tech Finance Board
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ position: 'relative' }}>
              ...
              <input
                type="text"
                placeholder="Search staff, order no..."
                value={searchTerm}
        ...
        <div style={{ padding: '20px 24px 10px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Eligible Staff</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#0f172a' }}>{summary.totalStaffEligible || filteredRecords.length}</div>
              <div style={{ fontSize: '11px', color: '#0284c7' }}>Sanction Orders Issued</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Previous Basic Base</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#0f172a' }}>₹{(summary.totalPreviousBasic || 0).toLocaleString('en-IN')}</div>
              <div style={{ fontSize: '11px', color: '#64748b' }}>Pre-Increment Total</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Increment Outflow</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#16a34a' }}>+₹{(summary.totalIncrementOutflow || 0).toLocaleString('en-IN')}</div>
              <div style={{ fontSize: '11px', color: '#16a34a' }}>Avg: +₹{summary.averageIncrement || 0}/staff</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Revised Basic Base</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#a21caf' }}>₹{(summary.totalRevisedBasic || 0).toLocaleString('en-IN')}</div>
              <div style={{ fontSize: '11px', color: '#a21caf' }}>3.00% Standard Hike</div>
            </div>
          </div>
        </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0', color: '#475569' }}>
                    ...
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right' }}>Previous Basic (₹)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'center' }}>Rate (%)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right', color: '#16a34a' }}>Increment (₹)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '700', textAlign: 'right', color: '#0369a1' }}>Revised Basic (₹)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600' }}>Order Number</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600' }}>Effective Date</th>
                  ...
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={12} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                        {loading ? 'Fetching increment records...' : 'No increment records found.'}
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((r, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                        ...
                        <td style={{ padding: '10px 14px', textAlign: 'right', color: '#64748b' }}>₹{(r.previousBasic || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'center', fontWeight: '600', color: '#d97706' }}>{r.incrementRate}%</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '600', color: '#16a34a' }}>+₹{(r.incrementAmount || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '700', color: '#0369a1' }}>₹{(r.revisedBasic || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', fontFamily: 'monospace', color: '#475569' }}>{r.orderNo}</td>
                        <td style={{ padding: '10px 14px', color: '#64748b' }}>{r.effectiveDate}</td>
                ...
                {filteredRecords.length > 0 && (
                  <tfoot>
                    <tr style={{ backgroundColor: '#f1f5f9', borderTop: '2px solid #cbd5e1', fontWeight: '700', color: '#0f172a' }}>
                      <td colSpan={5} style={{ padding: '12px 14px', textAlign: 'right' }}>Total Basic Base:</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right' }}>₹{(summary.totalPreviousBasic || 0).toLocaleString('en-IN')}</td>
                      <td></td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#16a34a' }}>+₹{(summary.totalIncrementOutflow || 0).toLocaleString('en-IN')}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#0369a1' }}>₹{(summary.totalRevisedBasic || 0).toLocaleString('en-IN')}</td>
                      <td colSpan={3}></td>
                    </tr>
                  </tfoot>
                )}
