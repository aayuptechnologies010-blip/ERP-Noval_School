...
import {
  ...
  AlertCircle, Sparkles, MessageSquare, Send, CheckCheck,
  ChevronLeft, ChevronRight, Filter, Clock, Phone,
  Building2, XCircle
} from 'lucide-react';
...
export default function SMSReport() {
  ...
  const fetchReport = async () => {
    try {
      ...
      if (deliveryStatus && !deliveryStatus.includes('All')) params.append('status', deliveryStatus);
      if (department && !department.includes('All')) params.append('department', department);
      if (searchTerm) params.append('search', searchTerm);

      const res = await fetch(`${API_BASE}/api/salary-structure/sms-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch SMS Dispatch report');
      }
    } catch (err) {
      console.error(err);
  ...
  const filteredRecords = records.filter(r => {
    ...
    return (
      ...
      (r.mobileNo && r.mobileNo.toLowerCase().includes(s)) ||
      (r.gatewayRef && r.gatewayRef.toLowerCase().includes(s))
    );
  });
  ...
  const exportCSV = () => {
    ...
    const csvHeaders = [
      'Sr No', 'SMS ID', 'Emp ID', 'Staff Name', 'Mobile No',
      'Net Salary (₹)', 'Message Content', 'Timestamp', 'Gateway Ref', 'Status'
    ];
    const rows = filteredRecords.map(r => [
      ...
      `"${r.mobileNo || ''}"`,
      r.netSalary,
      `"${r.messageText ? r.messageText.replace(/"/g, '""') : ''}"`,
      `"${r.sentTimestamp || ''}"`,
      `"${r.gatewayRef || ''}"`,
      `"${r.deliveryStatus || ''}"`
    ]);
    ...
    link.setAttribute('download', `Ayup_Tech_Salary_SMS_Report_${monthYear}.csv`);
      ...
      <div style={{
        ...
        <div style={{ padding: '20px', borderBottom: '1px solid #e2e8f0', background: 'linear-gradient(135deg, #0f172a, #1e293b)', color: 'white' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600, letterSpacing: '0.3px' }}>SMS Gateway Filter</h3>
          </div>
          <p style={{ margin: '6px 0 0 0', fontSize: '11px', color: '#94a3b8' }}>Ayup Tech SMS Notifications</p>
        </div>
        ...
        <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', flex: 1 }}>
          ...
          <div>
            <label style={{ fontSize: '12px', fontWeight: 600, color: '#475569', marginBottom: '6px', display: 'block' }}>Delivery Status</label>
            <select
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#16a34a', fontWeight: 600, background: '#dcfce7', padding: '2px 8px', borderRadius: '4px' }}>
                <Sparkles size={12} /> Ayup Tech SMS Delivery
              </span>
            ...
            <h1 style={{ margin: '4px 0 0 0', fontSize: '20px', fontWeight: 700, color: '#0f172a' }}>
              Salary SMS Dispatch Log & Delivery Verification
            </h1>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            >
              <Printer size={15} /> Print Dispatch Log
            </button>
          </div>
        </div>
        ...
        <div style={{ padding: '16px 24px 0 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Total SMS Dispatched</span>
              <div style={{ padding: '6px', borderRadius: '8px', backgroundColor: '#e0f2fe', color: '#0284c7' }}><Send size={16} /></div>
            </div>
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#0f172a', marginTop: '6px' }}>
              {summary.totalSMS ?? records.length}
            </div>
            <div style={{ fontSize: '11px', color: '#0284c7', marginTop: '2px', fontWeight: 500 }}>
              Ayup Tech Gateway Log
            </div>
          </div>

          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Delivered Successfully</span>
              <div style={{ padding: '6px', borderRadius: '8px', backgroundColor: '#dcfce7', color: '#16a34a' }}><CheckCheck size={16} /></div>
            </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Success Rate</span>
              <div style={{ padding: '6px', borderRadius: '8px', backgroundColor: '#fef3c7', color: '#d97706' }}><Sparkles size={16} /></div>
            </div>
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#d97706', marginTop: '6px' }}>
              {summary.deliverySuccessRate || '98.5%'}
            </div>
            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>
              High Institutional Delivery Reliability
            </div>
          </div>

          <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', padding: '14px 18px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '12px', fontWeight: 600, color: '#64748b' }}>Pending / Failed</span>
              <div style={{ padding: '6px', borderRadius: '8px', backgroundColor: '#f1f5f9', color: '#475569' }}><Clock size={16} /></div>
            </div>
            <div style={{ fontSize: '18px', fontWeight: 700, color: '#0f172a', marginTop: '6px' }}>
              {summary.pendingCount || 0} Pend • {summary.failedCount || 0} Fail
            </div>
            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>
              {summary.gatewayProvider || 'Ayup Tech Gateway'}
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
          <div style={{ position: 'relative', width: '320px' }}>
            ...
            <input
              type="text"
              placeholder="Search staff, mobile, gateway ref..."
              value={searchTerm}
          ...
          <div style={{ fontSize: '12px', color: '#64748b', fontWeight: 500 }}>
            Showing <b>{filteredRecords.length}</b> SMS dispatch logs
          </div>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '12px' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#475569', fontWeight: 600 }}>
                  ...
                  <th style={{ padding: '12px 14px' }}>SMS Ref</th>
                  ...
                  <th style={{ padding: '12px 14px', width: '40%' }}>SMS Message Content</th>
                  <th style={{ padding: '12px 14px' }}>Dispatched At</th>
                ...
                {filteredRecords.length > 0 ? (
                  filteredRecords.map((r, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: '11px 14px', fontFamily: 'monospace', fontWeight: 600, color: '#0284c7' }}>
                          {r.smsId}
                        </td>
                        <td style={{ padding: '11px 14px', fontWeight: 600, color: '#0f172a' }}>
                          <div>{r.staffName}</div>
                          <div style={{ fontSize: '11px', color: '#64748b', fontWeight: 400 }}>{r.department}</div>
                        </td>
                        ...
                        <td style={{ padding: '11px 14px', color: '#475569', fontSize: '11px', lineHeight: 1.4 }}>
                          {r.messageText}
                        </td>
                        <td style={{ padding: '11px 14px', color: '#64748b', fontSize: '11px' }}>
                          {r.sentTimestamp}
                        </td>
                        ...
                        <td style={{ padding: '11px 14px' }}>
                          <span style={{
                            ...
                            fontSize: '11px',
                            fontWeight: 600,
                            backgroundColor: isDelivered ? '#dcfce7' : isPending ? '#fef3c7' : '#fee2e2',
                            color: isDelivered ? '#15803d' : isPending ? '#b45309' : '#b91c1c'
                          }}>
                            {isDelivered ? <CheckCheck size={11} /> : isPending ? <Clock size={11} /> : <XCircle size={11} />}
                            {r.deliveryStatus}
                          </span>
                        </td>
                      </tr>
                  ...
                  <tr>
                    <td colSpan={9} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                      {loading ? 'Loading SMS dispatch records...' : 'No SMS records found.'}
                    </td>
                  </tr>
                )}
              </tbody>
