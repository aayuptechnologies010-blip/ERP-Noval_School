...
import {
  ...
  ChevronLeft, ChevronRight, Filter, Building2, Clock
} from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';

export default function DateRangeRetirementReport() {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/salary-structure/date-range-retirement-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch Date Range Retirement report');
      }
    } catch (err) {
      console.error(err);
  ...
  const filteredRecords = records.filter(r => {
    ...
    return (
      ...
      (r.retirementDate && r.retirementDate.toLowerCase().includes(s)) ||
  ...
  const exportCSV = () => {
    ...
    const headersList = [
      '#', 'Emp ID', 'Staff Name', 'Department', 'Designation', 'DOB',
      'Superannuation Age', 'Retirement Date', 'Remaining Tenure', 'Pension Eligibility', 'Settlement Status'
    ];
    ...
    link.setAttribute('download', `Date_Range_Retirement_Report_AyupTech.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showNotif('success', 'Retirement Report CSV downloaded successfully');
  };
      ...
      <div style={{
        ...
        {isSidebarOpen && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', height: '100%' }}>
            ...
            <div className="form-group">
              <label style={{ fontSize: '12px', fontWeight: '600', color: '#4a5568', marginBottom: '6px', display: 'block' }}>From Date</label>
              <input
                type="date"
                value={startDate}
                onChange={e => setStartDate(e.target.value)}
                style={{ width: '100%', padding: '8px 10px', borderRadius: '6px', border: '1px solid #cbd5e0', fontSize: '12px' }}
              />
            </div>

            <div className="form-group">
              <label style={{ fontSize: '12px', fontWeight: '600', color: '#4a5568', marginBottom: '6px', display: 'block' }}>To Date</label>
              <input
                type="date"
                value={endDate}
            ...
            <div style={{ marginTop: '10px' }}>
              ...
              >
                <Eye size={16} /> {loading ? 'Filtering...' : 'Show Retirements'}
              </button>
            </div>
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowX: 'hidden' }}>
        ...
        <div style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '700', color: '#0f172a' }}>
                  Retirement Forecast & Date Range Report
                </h2>
                <span style={{
                  ...
                  <Sparkles size={12} /> {startDate} to {endDate}
                </span>
                <span style={{
              ...
              <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                Superannuation Tracker & Succession Planning • {summary.superannuationRule || 'Age 60 Criterion'}
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ position: 'relative' }}>
              ...
              <input
                type="text"
                placeholder="Search staff, retirement date..."
                value={searchTerm}
        ...
        <div style={{ padding: '20px 24px 10px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Retiring Staff Count</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#0f172a' }}>{summary.totalRetiringStaff || filteredRecords.length}</div>
              <div style={{ fontSize: '11px', color: '#0284c7' }}>Within Selected Range</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Teaching Cadre</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#16a34a' }}>{summary.teachingCount || 0} Faculty</div>
              <div style={{ fontSize: '11px', color: '#16a34a' }}>Academic Faculty</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Non-Teaching Cadre</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#d97706' }}>{summary.nonTeachingCount || 0} Staff</div>
              <div style={{ fontSize: '11px', color: '#d97706' }}>Admin & Technical</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Superannuation Age</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#a21caf' }}>60 Years</div>
              <div style={{ fontSize: '11px', color: '#a21caf' }}>Statutory Benchmark</div>
            </div>
          </div>
        </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0', color: '#475569' }}>
                    ...
                    <th style={{ padding: '12px 14px', fontWeight: '600' }}>DOB</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'center' }}>Superannuation Age</th>
                    <th style={{ padding: '12px 14px', fontWeight: '700', color: '#d97706' }}>Retirement Date</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'center' }}>Remaining Service</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600' }}>Pension Eligibility</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'center' }}>Settlement Status</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={11} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                        {loading ? 'Filtering retirement records...' : 'No staff retiring in this date range.'}
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((r, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                        ...
                        <td style={{ padding: '10px 14px', color: '#64748b' }}>{r.dob}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'center', color: '#64748b' }}>{r.superannuationAge} Yrs</td>
                        <td style={{ padding: '10px 14px', fontWeight: '700', color: '#d97706' }}>{r.retirementDate}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'center', fontWeight: '600', color: '#16a34a' }}>{r.remainingYears}</td>
                        <td style={{ padding: '10px 14px', color: '#475569' }}>{r.pensionEligibility}</td>
