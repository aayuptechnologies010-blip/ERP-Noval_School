...
export default function DefinePayScale() {
  ...
  const [formData, setFormData] = useState({ scale: '', isActive: true });
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/api/pay-scales`, {
      ...
      if (res.ok) {
        ...
        showNotification('error', 'Failed to load Pay Scales');
      }
    } catch (err) {
      console.error('Error fetching pay scales:', err);
      showNotification('error', 'Server error while fetching pay scales');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenAdd = () => {
    setEditingId(null);
    setFormData({ scale: '', isActive: true });
    setShowModal(true);
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      setSubmitting(true);
      const url = editingId 
        ? `${API_BASE}/api/pay-scales/${editingId}`
        : `${API_BASE}/api/pay-scales`;
      ...
      if (res.ok) {
        showNotification('success', `Pay Scale ${editingId ? 'updated' : 'created'} successfully!`);
        setShowModal(false);
        fetchData();
      } else {
      ...
      console.error('Error saving pay scale:', err);
  ...
  const handleDelete = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${API_BASE}/api/pay-scales/${deleteConfirm._id}`, {
        method: 'DELETE',
        headers
      });

      if (res.ok) {
        showNotification('success', 'Pay Scale deleted successfully!');
  ...
  const handleExportCSV = () => {
    ...
    const headersLine = ['Sr No.', 'Pay Scale', 'Modified Date'];
    ...
    link.setAttribute('download', `Pay_Scales_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const filteredData = data.filter(row => {
    ...
    return row.scale && row.scale.toLowerCase().includes(q);
  });
    ...
    <div className="mail-template-container" style={{ padding: '20px' }}>
      ...
      <div className="mail-template-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
        ...
        <div className="header-buttons" style={{ display: 'flex', gap: '12px' }}>
          ...
          >
            <Plus size={16} /> Add New Pay Scale
          </button>
          <button 
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '600' }}>
                {editingId ? 'Edit Pay Scale' : 'Add New Pay Scale'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSave} style={{ padding: '20px' }}>
              <div style={{ marginBottom: '20px' }}>
                ...
                <input 
                  type="text" 
                  required
                  placeholder="e.g. 5300-150-10500, 9300-34800"
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                ...
                >
                  <Save size={16} /> {submitting ? 'Saving...' : 'Save Pay Scale'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#64748b', lineHeight: 1.5 }}>
              Are you sure you want to delete Pay Scale <strong>"{deleteConfirm.scale}"</strong>?
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button 
