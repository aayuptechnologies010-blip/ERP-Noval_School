...
export default function DefineDepartment() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/api/departments`, { headers: { 'Authorization': `Bearer ${token}` } });
      if (res.ok) {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`${API_BASE}/api/departments/${id}`, { method: 'DELETE', headers });
      if (res.ok) {
