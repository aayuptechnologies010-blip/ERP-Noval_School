...
export default function DefineFixation() {
  ...
  const [payScaleAmounts, setPayScaleAmounts] = useState([]);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [fixRes, scaleRes, gpRes, amtRes] = await Promise.all([
        fetch(`${API_BASE}/api/fixations`, { headers: { 'Authorization': `Bearer ${token}` } }),
      ...
      if (fixRes.ok) {
        ...
        setData(Array.isArray(fData) ? fData : []);
      }
      if (scaleRes.ok) {
      ...
      if (amtRes.ok) {
        ...
        setPayScaleAmounts(Array.isArray(aData) ? aData : []);
      }
    } catch (err) {
      console.error('Error loading fixations:', err);
      showNotification('error', 'Server error while loading fixation data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenAdd = () => {
    setEditingId(null);
    const defaultScale = payScales.length > 0 ? payScales[0].scale : '';
    const defaultGp = gradePays.length > 0 ? gradePays[0].amount : 2400;
    const defaultAmt = payScaleAmounts.length > 0 ? payScaleAmounts[0].amount : 5300;
  ...
  const handleOpenEdit = (item) => {
    ...
    setFormData({
      scale: item.scale || '',
      basicPercent: item.basicPercent !== undefined ? item.basicPercent : 50.00,
      daPercent: item.daPercent !== undefined ? item.daPercent : 21.00,
      gradePay: item.gradePay !== undefined ? item.gradePay : 2400.00,
      payScaleAmount: item.payScaleAmount !== undefined ? item.payScaleAmount : 5300.00,
  ...
  const handleSave = async (e) => {
    ...
    try {
      setSubmitting(true);
      const url = editingId 
        ? `${API_BASE}/api/fixations/${editingId}`
        : `${API_BASE}/api/fixations`;
      ...
      if (res.ok) {
        showNotification('success', `Fixation ${editingId ? 'updated' : 'created'} successfully!`);
        setShowModal(false);
        fetchData();
      } else {
      ...
      console.error('Error saving fixation:', err);
  ...
  const handleDelete = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${API_BASE}/api/fixations/${deleteConfirm._id}`, {
        method: 'DELETE',
        headers
      });

      if (res.ok) {
        showNotification('success', 'Fixation deleted successfully!');
      ...
      console.error('Error deleting fixation:', err);
  ...
  const handleExportCSV = () => {
    ...
    const headersLine = ['Sl No.', 'Pay Scale', 'Basic (%)', 'DA (%)', 'Grade Pay', 'Pay Scale Amount'];
    const rows = data.map((item, i) => [
      i + 1,
      `"${item.scale}"`,
      `${item.basicPercent}%`,
      `${item.daPercent}%`,
      item.gradePay !== undefined ? item.gradePay.toFixed(2) : '0.00',
      item.payScaleAmount !== undefined ? item.payScaleAmount.toFixed(2) : '0.00'
    ]);
    ...
    link.setAttribute('download', `Fixations_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const filteredData = data.filter(row => {
    ...
    return (
      ...
      String(row.gradePay).includes(q) ||
      String(row.payScaleAmount).includes(q) ||
      String(row.basicPercent).includes(q) ||
      String(row.daPercent).includes(q)
    );
  });
        ...
        <table className="mail-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              ...
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Basic (%)</th>
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>DA (%)</th>
              <th style={{ textAlign: 'right', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Grade Pay</th>
              <th style={{ textAlign: 'right', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Pay Scale Amount</th>
              ...
              currentRows.map((row, i) => (
                ...
                >
                  ...
                  <td style={{ textAlign: 'center', padding: '12px', fontSize: '13px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '12px', backgroundColor: '#f1f5f9', color: '#334155', fontWeight: '600' }}>
                      {row.basicPercent}%
                    </span>
                  </td>
                  <td style={{ textAlign: 'center', padding: '12px', fontSize: '13px' }}>
                    <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '12px', backgroundColor: '#e0f2fe', color: '#0369a1', fontWeight: '600' }}>
                      {row.daPercent}%
                    </span>
                  </td>
                  <td style={{ textAlign: 'right', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#0f172a' }}>
                    ₹{Number(row.gradePay).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </td>
                  <td style={{ textAlign: 'right', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#0f172a' }}>
                    ₹{Number(row.payScaleAmount).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </td>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '600' }}>
                {editingId ? 'Edit Fixation' : 'Add New Fixation'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSave} style={{ padding: '20px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '15px' }}>
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setFormData({ ...formData, basicPercent: Number(e.target.value) })}
                    style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1' }}
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setFormData({ ...formData, daPercent: Number(e.target.value) })}
                    style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1' }}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '20px' }}>
                <div>
                  ...
                  <select 
                    ...
                    onChange={(e) => setFormData({ ...formData, gradePay: Number(e.target.value) })}
                    style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1' }}
                  >
                    {gradePays.map(g => (
                      <option key={g._id} value={g.amount}>₹{g.amount}</option>
                    ))}
                    {!gradePays.some(g => g.amount === formData.gradePay) && (
                      <option value={formData.gradePay}>₹{formData.gradePay}</option>
                    )}
                  </select>
                </div>
                <div>
                  ...
                  <input 
                    type="number" 
                    step="any"
                    value={formData.payScaleAmount}
                    onChange={(e) => setFormData({ ...formData, payScaleAmount: Number(e.target.value) })}
                    style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1' }}
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                ...
                >
                  <Save size={16} /> {submitting ? 'Saving...' : 'Save Fixation'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#64748b', lineHeight: 1.5 }}>
              Are you sure you want to delete Fixation for scale <strong>"{deleteConfirm.scale}"</strong> (Grade Pay: ₹{deleteConfirm.gradePay})?
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button 
