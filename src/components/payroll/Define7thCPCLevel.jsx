...
export default function Define7thCPCLevel() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.ok) {
        ...
        showNotification('error', 'Failed to load 7th CPC levels');
      }
    } catch (err) {
      console.error('Error fetching CPC levels:', err);
  ...
  const handleOpenEdit = (item) => {
    ...
    setFormData({
      cpcLevel: item.cpcLevel || '',
      orderNo: item.orderNo || 1,
      noOfCells: item.noOfCells || 40,
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    if (!formData.cpcLevel.trim()) {
      alert('Pay Level Name is required');
      return;
    }
    ...
    try {
      setSubmitting(true);
      const url = editingId 
        ? `${API_BASE}/api/cpc-levels/${editingId}`
        : `${API_BASE}/api/cpc-levels`;
      ...
      if (res.ok) {
        showNotification('success', `7th CPC Level ${editingId ? 'updated' : 'created'} successfully!`);
        setShowModal(false);
        fetchData();
      } else {
      ...
      console.error('Error saving CPC level:', err);
  ...
  const handleDelete = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${API_BASE}/api/cpc-levels/${deleteConfirm._id}`, {
        method: 'DELETE',
        headers
      });
      if (res.ok) {
        showNotification('success', '7th CPC Level deleted successfully!');
      ...
      console.error('Error deleting CPC level:', err);
  ...
  const handleExportCSV = () => {
    ...
    const headersLine = ['Sr No.', '7th CPC Level', 'Order No.', 'No. Of Cells', 'Amount', 'Created Date'];
    const rows = levels.map((item, i) => [
      i + 1,
      `"${item.cpcLevel}"`,
      item.orderNo || '',
      item.noOfCells || '',
    ...
    link.setAttribute('download', `7th_CPC_Levels_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const filteredLevels = levels.filter(item => {
    ...
    return (
      (item.cpcLevel && item.cpcLevel.toLowerCase().includes(q)) ||
  ...
  const currentRows = filteredLevels.slice(startIndex, startIndex + pageSize);
    ...
    <div className="mail-template-container" style={{ padding: '20px' }}>
      ...
      <div className="mail-template-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
        ...
        <div className="header-buttons" style={{ display: 'flex', gap: '12px' }}>
          ...
          >
            <Plus size={16} /> Add New Pay Level Name
          </button>
          <button 
        ...
        <table className="mail-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              <th style={{ width: '80px', textAlign: 'center', padding: '12px', fontSize: '13px', color: '#475569' }}>Sr No.</th>
              <th style={{ textAlign: 'left', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>7th CPC Level</th>
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Order No.</th>
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>No. Of Cells</th>
              <th style={{ textAlign: 'right', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Amount</th>
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Created Date</th>
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569', width: '100px' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px', color: '#64748b', fontSize: '13px', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <div>
          Showing {filteredLevels.length === 0 ? 0 : startIndex + 1} to {Math.min(startIndex + pageSize, filteredLevels.length)} of {filteredLevels.length} entries
        </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '600' }}>
                {editingId ? 'Edit 7th CPC Pay Level' : 'Add New Pay Level Name'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSave} style={{ padding: '20px' }}>
              <div style={{ marginBottom: '16px' }}>
                ...
                <input 
                  type="text" 
                  required
                  placeholder="e.g. Level 1, Level 2, Level 8"
                  value={formData.cpcLevel}
                  onChange={(e) => setFormData({ ...formData, cpcLevel: e.target.value })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px', marginBottom: '16px' }}>
                ...
                <div>
                  ...
                  <input 
                    type="number" 
                    value={formData.noOfCells}
                    onChange={(e) => setFormData({ ...formData, noOfCells: Number(e.target.value) })}
                    style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid #cbd5e1' }}
                  />
                </div>
              </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#64748b', lineHeight: 1.5 }}>
              Are you sure you want to delete 7th CPC Level <strong>"{deleteConfirm.cpcLevel}"</strong>? This will remove its cell matrix configuration as well.
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button 
