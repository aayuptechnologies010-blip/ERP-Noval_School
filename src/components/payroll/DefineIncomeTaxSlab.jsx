...
export default function DefineIncomeTaxSlab() {
  ...
  const [formData, setFormData] = useState({
    slabType: 'Tax payable in Existing Regime',
    groupName: 'For Male',
    groupSlNo: 1,
    lowerBound: 0,
    upperBound: 250000,
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/api/income-tax-slabs`, {
      ...
      if (res.ok) {
        ...
        showNotification('error', 'Failed to fetch Income Tax Slabs');
      }
    } catch (err) {
      console.error('Error fetching income tax slabs:', err);
      showNotification('error', 'Server error while fetching slabs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenEdit = (item) => {
    ...
    setFormData({
      slabType: item.slabType || 'Tax payable in Existing Regime',
      groupName: item.groupName || 'For Male',
      groupSlNo: item.groupSlNo || 1,
      lowerBound: item.lowerBound !== undefined ? item.lowerBound : 0,
      upperBound: item.upperBound !== undefined ? item.upperBound : 0,
  ...
  const handleSave = async (e) => {
    ...
    try {
      setSubmitting(true);
      const url = editingId 
        ? `${API_BASE}/api/income-tax-slabs/${editingId}`
        : `${API_BASE}/api/income-tax-slabs`;
      ...
      if (res.ok) {
        showNotification('success', `Income Tax Slab ${editingId ? 'updated' : 'created'} successfully!`);
        setShowModal(false);
        fetchData();
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${API_BASE}/api/income-tax-slabs/${deleteConfirm._id}`, {
        method: 'DELETE',
        headers
      });

      if (res.ok) {
        showNotification('success', 'Income Tax Slab deleted successfully!');
  ...
  const handleExportCSV = () => {
    ...
    const headersLine = ['Sl No.', 'IT Slab Type', 'Group Name', 'Group Sl No.', 'Lower Bound', 'Upper Bound', 'Tax (%)'];
    const rows = data.map((item, i) => [
      i + 1,
      `"${item.slabType}"`,
      `"${item.groupName}"`,
      item.groupSlNo || '',
      item.lowerBound !== undefined ? item.lowerBound.toFixed(2) : '0.00',
      item.upperBound !== undefined ? item.upperBound.toFixed(2) : '0.00',
      item.taxRate !== undefined ? `${item.taxRate.toFixed(2)}%` : '0.00%'
    ]);
    ...
    link.setAttribute('download', `Income_Tax_Slabs_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const filteredData = data.filter(row => {
    ...
    return (
      (row.slabType && row.slabType.toLowerCase().includes(q)) ||
      (row.groupName && row.groupName.toLowerCase().includes(q)) ||
      String(row.lowerBound).includes(q) ||
      String(row.upperBound).includes(q) ||
      String(row.taxRate).includes(q)
    );
  });
        ...
        <table className="mail-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              ...
              <th style={{ textAlign: 'left', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>IT Slab Type</th>
              <th style={{ textAlign: 'left', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Group Name</th>
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Group Sl No.</th>
              <th style={{ textAlign: 'right', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Lower Bound</th>
              <th style={{ textAlign: 'right', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Upper Bound</th>
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Tax (%)</th>
            ...
            {loading ? (
              <tr>
                <td colSpan="8" style={{ textAlign: 'center', padding: '30px', color: '#64748b' }}>
                  Loading income tax slabs...
                </td>
              </tr>
            ) : currentRows.length === 0 ? (
              <tr>
                <td colSpan="8" style={{ textAlign: 'center', padding: '30px', color: '#94a3b8', backgroundColor: '#f8f9fa' }}>
                  No income tax slabs found.
                </td>
              </tr>
            ) : (
                ...
                >
                  ...
                  <td style={{ padding: '12px', fontSize: '13px', color: '#334155' }}>
                    <span style={{
                      ...
                      fontSize: '12px',
                      fontWeight: '500',
                      backgroundColor: row.slabType.includes('New') ? '#e0f2fe' : '#fef3c7',
                      color: row.slabType.includes('New') ? '#0369a1' : '#b45309'
                    }}>
                      {row.slabType}
                    </span>
                  </td>
                  ...
                  <td style={{ textAlign: 'right', padding: '12px', fontSize: '13px', color: '#0f172a' }}>
                    ₹{Number(row.lowerBound).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </td>
                  <td style={{ textAlign: 'right', padding: '12px', fontSize: '13px', color: '#0f172a' }}>
                    ₹{Number(row.upperBound).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                  </td>
                  <td style={{ textAlign: 'center', padding: '12px', fontSize: '13px' }}>
                    <span style={{
                      ...
                      fontSize: '12px',
                      fontWeight: '700',
                      backgroundColor: row.taxRate === 0 ? '#dcfce7' : '#fee2e2',
                      color: row.taxRate === 0 ? '#166534' : '#b91c1c'
                    }}>
                      {row.taxRate}%
                    </span>
                  </td>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '600' }}>
                {editingId ? 'Edit Income Tax Slab' : 'Add New Income Tax Slab'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSave} style={{ padding: '20px' }}>
              <div style={{ marginBottom: '15px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: '600', color: '#334155', marginBottom: '6px' }}>
                  IT Slab Type <span style={{ color: '#ef4444' }}>*</span>
                </label>
                <select 
                ...
                >
                  <option value="Tax payable in Existing Regime">Tax payable in Existing Regime</option>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '15px', marginBottom: '15px' }}>
                <div>
                  ...
                  <input 
                    type="text" 
                    required
                    placeholder="e.g. For Male, For Female, Senior Citizen"
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                ...
                >
                  <Save size={16} /> {submitting ? 'Saving...' : 'Save Slab'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#64748b', lineHeight: 1.5 }}>
              Are you sure you want to delete income tax slab for <strong>"{deleteConfirm.groupName}"</strong> (₹{deleteConfirm.lowerBound} - ₹{deleteConfirm.upperBound} @ {deleteConfirm.taxRate}%)?
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button 
