...
import {
  ...
  AlertCircle, Sparkles, Users, Award, ShieldCheck,
  ChevronLeft, ChevronRight, Filter, Building2, Milestone
} from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_BASE_URL || '';
export default function MACPListReport() {
  ...
  const [macpStage, setMacpStage] = useState('All');
  ...
  const fetchReport = async () => {
    try {
      ...
      if (macpStage && macpStage !== 'All') params.append('macpStage', macpStage);
      ...
      const res = await fetch(`${API_BASE}/api/salary-structure/macp-list-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch MACP list');
      }
    } catch (err) {
      console.error(err);
  ...
  const filteredRecords = records.filter(r => {
    ...
    return (
      ...
      (r.macpStage && r.macpStage.toLowerCase().includes(s)) ||
  ...
  const exportCSV = () => {
    ...
    const headersList = [
      ...
      'MACP Milestone', 'Current Pay Level', 'Upgraded Pay Level', 'Sanction Order', 'Grant Date', 'Status'
    ];
    ...
    link.setAttribute('download', `MACP_Career_Progression_List_AyupTech.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showNotif('success', 'MACP List CSV downloaded successfully');
  };
      ...
      <div style={{
        ...
        {isSidebarOpen && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', height: '100%' }}>
            ...
            <div className="form-group">
              <label style={{ fontSize: '12px', fontWeight: '600', color: '#4a5568', marginBottom: '6px', display: 'block' }}>MACP Milestone Stage</label>
              <select value={macpStage} onChange={e => setMacpStage(e.target.value)} style={{ width: '100%', padding: '8px 10px', borderRadius: '6px', border: '1px solid #cbd5e0', fontSize: '12px' }}>
                <option value="All">All Stages (10 / 20 / 30 Yrs)</option>
                <option value="1st MACP (10 Yrs)">1st MACP (10 Years)</option>
                <option value="2nd MACP (20 Yrs)">2nd MACP (20 Years)</option>
                <option value="3rd MACP (30 Yrs)">3rd MACP (30 Years)</option>
              </select>
            </div>
            ...
            <div style={{ marginTop: '10px' }}>
              ...
              >
                <Eye size={16} /> {loading ? 'Fetching...' : 'Show MACP Milestone List'}
              </button>
            </div>
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowX: 'hidden' }}>
        ...
        <div style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '700', color: '#0f172a' }}>
                  Modified Assured Career Progression (MACP) List
                </h2>
                <span style={{
                  ...
                  <Sparkles size={12} /> {data.screeningYear || '2026-2027'}
                </span>
                <span style={{
              ...
              <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                Financial Upgradations under MACP Scheme at 10, 20 & 30 Years of Regular Service • {data.screeningAuthority || 'Departmental Screening Committee'}
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ position: 'relative' }}>
              ...
              <input
                type="text"
                placeholder="Search staff, milestone, order..."
                value={searchTerm}
        ...
        <div style={{ padding: '20px 24px 10px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Total Milestones Due</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#0f172a' }}>{summary.totalMilestonesDue || filteredRecords.length}</div>
              <div style={{ fontSize: '11px', color: '#0284c7' }}>Eligible for Grade Upgrade</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>1st MACP (10 Yrs)</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#16a34a' }}>{summary.stage1Count || 0} Staff</div>
              <div style={{ fontSize: '11px', color: '#16a34a' }}>1st Stage Financial Hike</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>2nd MACP (20 Yrs)</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#d97706' }}>{summary.stage2Count || 0} Staff</div>
              <div style={{ fontSize: '11px', color: '#d97706' }}>2nd Stage Career Milestone</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>3rd MACP (30 Yrs)</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#a21caf' }}>{summary.stage3Count || 0} Staff</div>
              <div style={{ fontSize: '11px', color: '#a21caf' }}>Apex Career Milestone</div>
            </div>
          </div>
        </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0', color: '#475569' }}>
                    ...
                    <th style={{ padding: '12px 14px', fontWeight: '600', color: '#0369a1' }}>MACP Stage</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600' }}>Current Pay Level</th>
                    <th style={{ padding: '12px 14px', fontWeight: '700', color: '#16a34a' }}>Upgraded Pay Level</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600' }}>Sanction Order</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600' }}>Grant Date</th>
                  ...
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={12} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                        {loading ? 'Fetching MACP records...' : 'No MACP milestones found.'}
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((r, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                        ...
                        <td style={{ padding: '10px 14px', fontWeight: '600', color: '#0369a1' }}>{r.macpStage}</td>
                        <td style={{ padding: '10px 14px', color: '#64748b' }}>{r.currentPayLevel}</td>
                        <td style={{ padding: '10px 14px', fontWeight: '700', color: '#16a34a' }}>{r.upgradedPayLevel}</td>
                        <td style={{ padding: '10px 14px', fontFamily: 'monospace', color: '#475569' }}>{r.orderNo}</td>
                        <td style={{ padding: '10px 14px', color: '#64748b' }}>{r.grantDate}</td>
