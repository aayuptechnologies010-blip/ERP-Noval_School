...
export default function DefineGradePay() {
  ...
  const [formData, setFormData] = useState({ amount: 0, isActive: true });
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/api/grade-pays`, {
      ...
      if (res.ok) {
        ...
        showNotification('error', 'Failed to load Grade Pays');
      }
    } catch (err) {
      console.error('Error fetching grade pays:', err);
      showNotification('error', 'Server error while fetching grade pays');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      setSubmitting(true);
      const url = editingId 
        ? `${API_BASE}/api/grade-pays/${editingId}`
        : `${API_BASE}/api/grade-pays`;
      ...
      if (res.ok) {
        showNotification('success', `Grade Pay ${editingId ? 'updated' : 'created'} successfully!`);
        setShowModal(false);
        fetchData();
      } else {
      ...
      console.error('Error saving grade pay:', err);
  ...
  const handleDelete = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${API_BASE}/api/grade-pays/${deleteConfirm._id}`, {
        method: 'DELETE',
        headers
      });

      if (res.ok) {
        showNotification('success', 'Grade Pay deleted successfully!');
      ...
      console.error('Error deleting grade pay:', err);
  ...
  const handleExportCSV = () => {
    ...
    const headersLine = ['Sr No.', 'Amount (₹)', 'Modified Date'];
    ...
    link.setAttribute('download', `Grade_Pays_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const filteredData = data.filter(row => {
    ...
    return String(row.amount).includes(q);
  });
        ...
        <table className="mail-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              ...
              <th style={{ width: '100px', textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Action</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan="4" style={{ textAlign: 'center', padding: '30px', color: '#94a3b8', backgroundColor: '#f8f9fa' }}>
                  No grade pays found.
                </td>
              </tr>
            ) : (
              currentRows.map((row, i) => (
                ...
                >
                  ...
                  <td style={{ textAlign: 'right', padding: '12px', fontSize: '14px', fontWeight: '700', color: '#0f172a' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '600' }}>
                {editingId ? 'Edit Grade Pay' : 'Add New Grade Pay'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSave} style={{ padding: '20px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                ...
                >
                  <Save size={16} /> {submitting ? 'Saving...' : 'Save Grade Pay'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#64748b', lineHeight: 1.5 }}>
              Are you sure you want to delete Grade Pay <strong>"₹{deleteConfirm.amount}"</strong>?
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button 
