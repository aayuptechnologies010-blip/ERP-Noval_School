...
import {
  ...
  AlertCircle, Sparkles, DollarSign, Users, Award, ShieldCheck,
  ChevronLeft, ChevronRight, Filter, Building2, TrendingUp, CreditCard
} from 'lucide-react';

const API_BASE = import.meta.env.VITE_API_BASE_URL || '';
export default function AnnualSalaryPaidReport() {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/salary-structure/annual-salary-paid-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch annual salary report');
      }
    } catch (err) {
      console.error(err);
  ...
  const exportCSV = () => {
    ...
    const headersList = [
      '#', 'Emp ID', 'Staff Name', 'Designation', 'Department', 'Staff Type', 'Months Paid',
      'Annual Basic (₹)', 'Annual HRA (₹)', 'Annual DA (₹)', 'Annual Allowances (₹)',
      'Annual Gross (₹)', 'Annual PF (₹)', 'Annual TDS (₹)', 'Total Deductions (₹)',
      'Annual Net Paid (₹)', 'Bank Name', 'Account No', 'Status'
    ];
    const rows = filteredRecords.map((r, i) => [
      ...
      r.annualHra,
      r.annualDa,
      (r.annualConveyance || 0) + (r.annualSpecialAllowance || 0),
      r.annualGross,
      r.annualPf,
      r.annualTds,
      r.annualTotalDeductions,
      r.annualNetPaid,
      `"${r.bankName || ''}"`,
      `"${r.bankAccountNo || ''}"`,
      `"${r.status || 'Paid'}"`
    ...
    link.setAttribute('download', `Annual_Salary_Paid_Report_${financialYear}.csv`);
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ fontSize: '12px', color: '#718096', margin: '4px 0 0 0' }}>
                Comprehensive annual payroll statement with basic, allowances, statutory deductions and cumulative net disbursed
              </p>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ position: 'relative' }}>
                ...
                <input
                  type="text"
                  placeholder="Search staff, dept, bank..."
                  value={searchTerm}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '20px' }}>
            <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #10B981', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '12px', color: '#718096', fontWeight: '600' }}>TOTAL ANNUAL PAID</span>
                <DollarSign size={18} color="#10B981" />
              </div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#1a202c', marginTop: '6px' }}>
                ₹{(summary.totalAnnualNetPaid || 0).toLocaleString('en-IN')}
              </div>
              <span style={{ fontSize: '11px', color: '#10B981', fontWeight: '500' }}>Disbursed net salary</span>
            </div>
            ...
            <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #159BD7', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <TrendingUp size={18} color="#159BD7" />
              </div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#1a202c', marginTop: '6px' }}>
                ₹{(summary.totalAnnualGross || 0).toLocaleString('en-IN')}
              </div>
              <span style={{ fontSize: '11px', color: '#718096' }}>Cumulative earnings</span>
            </div>
            ...
            <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #EF4444', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              ...
              <span style={{ fontSize: '11px', color: '#718096' }}>Statutory taxes & PF</span>
            </div>
            ...
            <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #8B5CF6', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '12px', color: '#718096', fontWeight: '600' }}>AVG ANNUAL SALARY</span>
                <Award size={18} color="#8B5CF6" />
              </div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#1a202c', marginTop: '6px' }}>
                ₹{(summary.avgAnnualPackage || 0).toLocaleString('en-IN')}
              </div>
              <span style={{ fontSize: '11px', color: '#8B5CF6', fontWeight: '500' }}>Across active staff</span>
            </div>
          </div>

          {/* Master Annual Salary Table */}
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left', minWidth: '1100px' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#4a5568', fontWeight: '700' }}>
                    ...
                    <th style={{ padding: '12px 14px', textAlign: 'center' }}>Months</th>
                    <th style={{ padding: '12px 14px', textAlign: 'right' }}>Basic Pay (₹)</th>
                    ...
                    <th style={{ padding: '12px 14px', textAlign: 'right' }}>Annual Net Paid (₹)</th>
                  ...
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={17} style={{ padding: '30px', textAlign: 'center', color: '#a0aec0' }}>
                        No annual salary records match the selected criteria
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((r, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #edf2f7', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                        ...
                        <td style={{ padding: '12px 14px', textAlign: 'center' }}>
                          <span style={{ backgroundColor: '#e2e8f0', padding: '2px 8px', borderRadius: '10px', fontSize: '11px', fontWeight: '600' }}>
                            {r.monthsCount} / 12
                          </span>
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: '#4a5568' }}>
                          ₹{r.annualBasic.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: '#4a5568' }}>
                          ₹{r.annualDa.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: '#4a5568' }}>
                          ₹{r.annualHra.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: '#4a5568' }}>
                          ₹{((r.annualConveyance || 0) + (r.annualSpecialAllowance || 0)).toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: '600', color: '#1a202c' }}>
                          ₹{r.annualGross.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: '#e53e3e' }}>
                          ₹{r.annualPf.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', color: '#e53e3e' }}>
                          ₹{r.annualTds.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: '600', color: '#e53e3e' }}>
                          ₹{r.annualTotalDeductions.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: '700', color: '#10B981' }}>
                          ₹{r.annualNetPaid.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', color: '#718096', fontSize: '11px' }}>
                          <div>{r.bankName}</div>
                          <div style={{ fontFamily: 'monospace' }}>{r.bankAccountNo}</div>
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'center' }}>
                          <span style={{
                ...
                {filteredRecords.length > 0 && (
                  <tfoot>
                    <tr style={{ backgroundColor: '#f1f5f9', fontWeight: '700', borderTop: '2px solid #cbd5e0' }}>
                      <td colSpan={6} style={{ padding: '12px 14px', color: '#1a202c' }}>ANNUAL TOTALS</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#4a5568' }}>
                        ₹{filteredRecords.reduce((sum, r) => sum + r.annualBasic, 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#4a5568' }}>
                        ₹{filteredRecords.reduce((sum, r) => sum + r.annualDa, 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#4a5568' }}>
                        ₹{filteredRecords.reduce((sum, r) => sum + r.annualHra, 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#4a5568' }}>
                        ₹{filteredRecords.reduce((sum, r) => sum + (r.annualConveyance || 0) + (r.annualSpecialAllowance || 0), 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#1a202c' }}>
                        ₹{(summary.totalAnnualGross || 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#e53e3e' }}>
                        ₹{filteredRecords.reduce((sum, r) => sum + r.annualPf, 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#e53e3e' }}>
                        ₹{filteredRecords.reduce((sum, r) => sum + r.annualTds, 0).toLocaleString('en-IN')}
                      </td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#e53e3e' }}>
                        ₹{(summary.totalAnnualDeductions || 0).toLocaleString('en-IN')}
                      </td>
