...
import {
  ...
  ChevronLeft, ChevronRight, Filter, Building2, FileText
} from 'lucide-react';
...
export default function ProfessionalTaxReport() {
  ...
  const exportCSV = () => {
    ...
    const headersList = [
      ...
      'Monthly Gross (₹)', 'PT Slab Bracket', 'PT Deducted (₹)', 'Treasury Ref', 'Remittance Status', 'Month'
    ];
    ...
    link.setAttribute('download', `Professional_Tax_Report_${monthYear}.csv`);
          ...
          <div style={{
            ...
            <div>
              ...
              <p style={{ fontSize: '12px', color: '#718096', margin: '4px 0 0 0' }}>
                State government statutory professional tax deduction report, slab assessments and Treasury challan tracking
              </p>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '20px' }}>
            <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #159BD7', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '12px', color: '#718096', fontWeight: '600' }}>TOTAL PT DEDUCTED</span>
                <DollarSign size={18} color="#159BD7" />
              </div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#1a202c', marginTop: '6px' }}>
                ₹{(summary.totalPtDeducted || 0).toLocaleString('en-IN')}
              </div>
              <span style={{ fontSize: '11px', color: '#718096' }}>Deducted for {monthYear}</span>
            </div>
            ...
            <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #10B981', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '12px', color: '#718096', fontWeight: '600' }}>ASSESSED STAFF</span>
                <Users size={18} color="#10B981" />
              </div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#1a202c', marginTop: '6px' }}>
                {summary.assessedStaffCount || 0} / {summary.totalStaffCount || 0} Staff
              </div>
              <span style={{ fontSize: '11px', color: '#10B981', fontWeight: '500' }}>Eligible PT contributors</span>
            </div>
            ...
            <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #8B5CF6', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '12px', color: '#718096', fontWeight: '600' }}>TREASURY CHALLAN</span>
                <FileText size={18} color="#8B5CF6" />
              </div>
              <div style={{ fontSize: '15px', fontWeight: '700', color: '#1a202c', marginTop: '8px', fontFamily: 'monospace' }}>
                {summary.treasuryChallanNo || 'PT-GOV-UP/2026/TR-892'}
              </div>
              <span style={{ fontSize: '11px', color: '#8B5CF6', fontWeight: '500' }}>Govt. Remittance ID</span>
            </div>
            ...
            <div style={{ backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', borderLeft: '4px solid #047481', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '12px', color: '#718096', fontWeight: '600' }}>COMPLIANCE STATUS</span>
                <ShieldCheck size={18} color="#047481" />
              </div>
              <div style={{ fontSize: '18px', fontWeight: '700', color: '#047481', marginTop: '6px' }}>
                {summary.complianceStatus || '100% Remitted'}
              </div>
              <span style={{ fontSize: '11px', color: '#047481', fontWeight: '500' }}>State statutory compliance</span>
            </div>
          </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left', minWidth: '950px' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0', color: '#4a5568', fontWeight: '700' }}>
                    ...
                    <th style={{ padding: '12px 14px', textAlign: 'right' }}>Monthly Gross (₹)</th>
                    <th style={{ padding: '12px 14px' }}>PT Slab Bracket</th>
                    <th style={{ padding: '12px 14px', textAlign: 'right' }}>PT Deducted (₹)</th>
                    <th style={{ padding: '12px 14px' }}>Treasury Ref</th>
                  ...
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={11} style={{ padding: '30px', textAlign: 'center', color: '#a0aec0' }}>
                        No professional tax records match the selected criteria
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((r, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #edf2f7', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                        ...
                        <td style={{ padding: '12px 14px', color: '#718096' }}>{r.staffType}</td>
                        ...
                        <td style={{ padding: '12px 14px', color: '#4a5568' }}>
                          <span style={{ backgroundColor: '#edf2f7', padding: '2px 8px', borderRadius: '4px', fontSize: '11px' }}>
                            {r.ptSlab}
                          </span>
                        </td>
                        <td style={{ padding: '12px 14px', textAlign: 'right', fontWeight: '700', color: '#2b6cb0' }}>
                          ₹{r.ptDeduction.toLocaleString('en-IN')}
                        </td>
                        <td style={{ padding: '12px 14px', fontFamily: 'monospace', color: '#718096', fontSize: '11px' }}>
                          {r.remittanceRef}
                        </td>
