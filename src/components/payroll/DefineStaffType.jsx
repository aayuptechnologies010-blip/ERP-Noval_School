...
export default function DefineStaffType() {
  ...
  const [formData, setFormData] = useState({ type: '', hourly: false, showEcare: false });
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE}/api/staff-types`, { headers: { 'Authorization': `Bearer ${token}` } });
      if (res.ok) {
  ...
  const handleSave = async () => {
    try {
      const url = editingId ? `${API_BASE}/api/staff-types/${editingId}` : `${API_BASE}/api/staff-types`;
      const method = editingId ? 'PUT' : 'POST';
      const res = await fetch(url, { method, headers, body: JSON.stringify(formData) });
      if (res.ok) {
        setShowModal(false);
        setEditingId(null);
        setFormData({ type: '', hourly: false, showEcare: false });
        fetchData();
      }
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    try {
      const res = await fetch(`${API_BASE}/api/staff-types/${id}`, { method: 'DELETE', headers });
      if (res.ok) {
    ...
    <div className="mail-template-container">
      <div className="mail-template-header">
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button className="add-template-btn" onClick={() => { setEditingId(null); setFormData({ type: '', hourly: false, showEcare: false }); setShowModal(true); }}>
        ...
        <table className="mail-table">
          <thead>
            <tr>
              ...
              <th>Staff Type</th>
              <th style={{ width: '150px', textAlign: 'center' }}>Hourly</th>
              <th style={{ width: '150px', textAlign: 'center' }}>Show E-care</th>
            ...
            {loading ? (
              ...
              data.map((row, index) => (
                <tr key={row._id} className={index % 2 === 0 ? 'row-even' : 'row-odd'}>
                  ...
                  <td>{row.type}</td>
                  <td style={{ textAlign: 'center' }}>{row.hourly ? 'Yes' : 'No'}</td>
                  <td style={{ textAlign: 'center' }}>{row.showEcare ? 'Yes' : 'No'}</td>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ backgroundColor: '#fff', borderRadius: '8px', padding: '24px', width: '400px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ margin: 0 }}>{editingId ? 'Edit Staff Type' : 'Add Staff Type'}</h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '20px' }}>
                <label><input type="checkbox" checked={formData.hourly} onChange={(e) => setFormData({...formData, hourly: e.target.checked})} /> Hourly</label>
                <label><input type="checkbox" checked={formData.showEcare} onChange={(e) => setFormData({...formData, showEcare: e.target.checked})} /> Show E-care</label>
              </div>
            </div>
