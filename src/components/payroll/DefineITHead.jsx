...
export default function DefineITHead() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [headsRes, groupsRes] = await Promise.all([
        fetch(`${API_BASE}/api/it-heads`, { headers: { 'Authorization': `Bearer ${token}` } }),
        fetch(`${API_BASE}/api/it-head-groups`, { headers: { 'Authorization': `Bearer ${token}` } })
      ]);

      if (headsRes.ok) {
        const hData = await headsRes.json();
        setData(Array.isArray(hData) ? hData : []);
      }
      if (groupsRes.ok) {
      ...
      console.error('Error fetching IT heads:', err);
      showNotification('error', 'Server error while fetching IT heads');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenAdd = () => {
    ...
    setFormData({
      groupId: defaultGroup ? defaultGroup._id : '',
      groupName: defaultGroup ? defaultGroup.groupName : '',
      slNo: (data.length || 0) + 1,
      headName: '',
      reportName: '',
      maxRebateLimit: defaultGroup ? defaultGroup.maxRebateLimit : 0,
      isActive: true
    });
    setShowModal(true);
  };
  ...
  const handleGroupChange = (e) => {
    ...
    const foundGroup = groups.find(g => g.groupName === selectedGName);
    setFormData(prev => ({
      ...prev,
      groupName: selectedGName,
      groupId: foundGroup ? foundGroup._id : '',
      maxRebateLimit: foundGroup && prev.maxRebateLimit === 0 ? foundGroup.maxRebateLimit : prev.maxRebateLimit
    }));
  };
  ...
  const handleSave = async (e) => {
    ...
    if (!formData.groupName) {
      alert('Please select an IT Head Group');
      return;
    }
    ...
    try {
      setSubmitting(true);
      const url = editingId 
        ? `${API_BASE}/api/it-heads/${editingId}`
        : `${API_BASE}/api/it-heads`;
      ...
      if (res.ok) {
        showNotification('success', `IT Head ${editingId ? 'updated' : 'created'} successfully!`);
        setShowModal(false);
        fetchData();
      } else {
      ...
      console.error('Error saving IT head:', err);
  ...
  const handleDelete = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`${API_BASE}/api/it-heads/${deleteConfirm._id}`, {
        method: 'DELETE',
        headers
      });

      if (res.ok) {
        showNotification('success', 'IT Head deleted successfully!');
      ...
      console.error('Error deleting IT head:', err);
  ...
  const handleExportCSV = () => {
    ...
    const headersLine = ['Sr No.', 'Group Name', 'Sl No.', 'Head Name', 'Report Name', 'Max Rebate Limit', 'Modify Date'];
    ...
    link.setAttribute('download', `IT_Heads_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const filteredData = data.filter(row => {
    ...
    return (
      ...
      (row.headName && row.headName.toLowerCase().includes(q)) ||
      (row.reportName && row.reportName.toLowerCase().includes(q)) ||
        ...
        <table className="mail-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
              ...
              <th style={{ textAlign: 'left', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Head Name</th>
              <th style={{ textAlign: 'left', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Report Name</th>
              <th style={{ textAlign: 'right', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Max Rebate Limit</th>
              <th style={{ textAlign: 'center', padding: '12px', fontSize: '13px', fontWeight: '600', color: '#475569' }}>Modify Details</th>
            ...
            {loading ? (
              ...
              currentRows.map((row, i) => (
                ...
                >
                  ...
                  <td style={{ padding: '12px', fontSize: '13px', fontWeight: '600', color: '#0369a1' }}>
                    <span style={{
                      padding: '3px 8px',
                      borderRadius: '4px',
                      fontSize: '12px',
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: '600' }}>
                {editingId ? 'Edit IT Head' : 'Add New IT Head'}
              </h3>
              <button 
            ...
            <form onSubmit={handleSave} style={{ padding: '20px' }}>
              <div style={{ marginBottom: '15px' }}>
                ...
                >
                  <option value="">-- Select IT Head Group --</option>
                  {groups.map(g => (
                    <option key={g._id} value={g.groupName}>
                      {g.groupName} (Max: ₹{g.maxRebateLimit})
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '15px', marginBottom: '15px' }}>
                <div>
                  ...
                  <input 
                    type="text" 
                    required
                    placeholder="e.g. 80C LIC Premium, 80D Mediclaim"
              ...
              <div style={{ marginBottom: '15px' }}>
                ...
                <input 
                  type="text" 
                  placeholder="e.g. LIC, Mediclaim, HRA"
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                ...
                >
                  <Save size={16} /> {submitting ? 'Saving...' : 'Save Head'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#64748b', lineHeight: 1.5 }}>
              Are you sure you want to delete IT Head <strong>"{deleteConfirm.headName}"</strong> (Group: {deleteConfirm.groupName})?
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              <button 
