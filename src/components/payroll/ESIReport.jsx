...
import {
  ...
  ChevronLeft, ChevronRight, Filter, Building2, HeartHandshake
} from 'lucide-react';
...
export default function ESIReport() {
  ...
  const fetchReport = async () => {
    try {
      ...
      const res = await fetch(`${API_BASE}/api/salary-structure/esi-report?${params.toString()}`, { headers });
      if (res.ok) {
        const json = await res.json();
        setData(json);
      } else {
        showNotif('error', 'Failed to fetch ESI report');
      }
    } catch (err) {
      console.error(err);
  ...
  const exportCSV = () => {
    ...
    const headersList = [
      '#', 'Emp ID', 'Staff Name', 'ESI IP Number', 'Department', 'Working Days',
      'Gross Salary (₹)', 'ESI Wages (₹)', 'Employee Share 0.75% (₹)', 'Employer Share 3.25% (₹)', 'Total Remittance (₹)', 'Status', 'Month'
    ];
    ...
    link.setAttribute('download', `ESI_Monthly_Report_${monthYear}_AyupTech.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    showNotif('success', 'ESI Report CSV downloaded successfully');
  };
      ...
      <div style={{
        ...
        {isSidebarOpen && (
          <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', overflowY: 'auto', height: '100%' }}>
            ...
            <div className="form-group">
              <label style={{ fontSize: '12px', fontWeight: '600', color: '#4a5568', marginBottom: '6px', display: 'block' }}>Wage Month</label>
            ...
            <div style={{ marginTop: '10px' }}>
              ...
              >
                <Eye size={16} /> {loading ? 'Loading...' : 'Show ESI Return'}
              </button>
            </div>
          </div>
        )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflowX: 'hidden' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '700', color: '#0f172a' }}>
                  Employees’ State Insurance (ESI) Monthly Report
                </h2>
                <span style={{
              ...
              <p style={{ margin: '2px 0 0', fontSize: '12px', color: '#64748b' }}>
                ESI Scheme Remittance (Employee 0.75% + Employer 3.25%) • ESIC Code: {data.esiEstablishmentCode || '01000192840000101'}
              </p>
            </div>
          </div>
        ...
        <div style={{ padding: '20px 24px 10px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Covered IP Staff</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#0f172a' }}>{summary.coveredStaffCount || 0} / {summary.totalStaff || filteredRecords.length}</div>
              <div style={{ fontSize: '11px', color: '#0284c7' }}>Insured Persons (IP)</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>ESI Wages Base</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#0f172a' }}>₹{(summary.totalEsiWages || 0).toLocaleString('en-IN')}</div>
              <div style={{ fontSize: '11px', color: '#64748b' }}>Wages u/s 2(22) of ESI Act</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Employee ESI (0.75%)</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#16a34a' }}>₹{(summary.totalEeContribution || 0).toLocaleString('en-IN')}</div>
              <div style={{ fontSize: '11px', color: '#16a34a' }}>Insured Person Deduction</div>
            </div>
          </div>
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '16px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              <div style={{ fontSize: '11px', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>Total ESI Remittance</div>
              <div style={{ fontSize: '20px', fontWeight: '700', color: '#a21caf' }}>₹{(summary.totalRemittance || 0).toLocaleString('en-IN')}</div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', textAlign: 'left' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0', color: '#475569' }}>
                    ...
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right' }}>ESI Wages (₹)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right', color: '#0369a1' }}>EE 0.75% (₹)</th>
                    <th style={{ padding: '12px 14px', fontWeight: '600', textAlign: 'right', color: '#059669' }}>ER 3.25% (₹)</th>
                  ...
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={12} style={{ padding: '40px', textAlign: 'center', color: '#94a3b8' }}>
                        {loading ? 'Fetching ESI records from server...' : 'No ESI records found.'}
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((r, idx) => (
                      <tr key={idx} style={{ borderBottom: '1px solid #f1f5f9', backgroundColor: idx % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                        ...
                        <td style={{ padding: '10px 14px', textAlign: 'center', color: '#64748b' }}>{r.workingDays}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', color: '#64748b' }}>₹{(r.grossSalary || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '600', color: '#0f172a' }}>₹{(r.esiWages || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '600', color: '#0369a1' }}>₹{(r.eeContribution || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '600', color: '#059669' }}>₹{(r.erContribution || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: '700', color: '#7c3aed' }}>₹{(r.totalRemittance || 0).toLocaleString('en-IN')}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'center' }}>
                          <span style={{
                            padding: '3px 8px', borderRadius: '12px', fontSize: '10px', fontWeight: '600',
                            backgroundColor: r.status === 'Covered' || r.eeContribution > 0 ? '#dcfce7' : '#f1f5f9',
                            color: r.status === 'Covered' || r.eeContribution > 0 ? '#166534' : '#64748b'
                          }}>
                            {r.status}
                          </span>
                        </td>
                ...
                {filteredRecords.length > 0 && (
                  <tfoot>
                    <tr style={{ backgroundColor: '#f1f5f9', borderTop: '2px solid #cbd5e1', fontWeight: '700', color: '#0f172a' }}>
                      <td colSpan={7} style={{ padding: '12px 14px', textAlign: 'right' }}>Total Remittance:</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right' }}>₹{(summary.totalEsiWages || 0).toLocaleString('en-IN')}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#0369a1' }}>₹{(summary.totalEeContribution || 0).toLocaleString('en-IN')}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#059669' }}>₹{(summary.totalErContribution || 0).toLocaleString('en-IN')}</td>
                      <td style={{ padding: '12px 14px', textAlign: 'right', color: '#7c3aed' }}>₹{(summary.totalRemittance || 0).toLocaleString('en-IN')}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                )}
