...
export default function ManageUpdateAddressBlood() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/update-address-blood`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        await fetch(`${API_BASE}/api/update-address-blood/${editItem._id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: inputValue })
        });
      } else {
        await fetch(`${API_BASE}/api/update-address-blood`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: inputValue })
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      try {
        await fetch(`${API_BASE}/api/update-address-blood/${id}`, { method: 'DELETE' });
        fetchData();
      } catch (err) {
        console.error(err);
      }
