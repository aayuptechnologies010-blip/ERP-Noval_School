...
export default function EditAlbum() {
  ...
  useEffect(() => {
    const fetchAlbum = async () => {
      try {
        ...
        toast.error("An error occurred while fetching album");
        navigate('/dashboard/photos');
      } finally {
        setFetching(false);
      }
  ...
  if (fetching) {
    return <div className="flex-1 bg-gray-50 rounded-tl-3xl p-8 flex items-center justify-center text-gray-500">Loading album data...</div>;
  }

  return (
    <div className="flex-1 bg-gray-50 rounded-tl-3xl p-8 overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-xl font-bold text-blue-900">Edit Album</h1>
        <button 
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col items-center mb-8 p-6 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
            <div className="w-48 h-32 bg-gray-200 flex flex-col items-center justify-center text-gray-400 mb-4 overflow-hidden rounded shadow-sm">
              {coverImage ? (
                <img src={URL.createObjectURL(coverImage)} alt="New Cover Preview" className="w-full h-full object-cover" />
              ) : existingImageUrl ? (
