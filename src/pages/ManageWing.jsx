...
function ManageWing() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = editItem 
        ? `${import.meta.env.VITE_API_BASE_URL}/api/wings/${editItem._id}`
  ...
  return (
    <div style={{ flex: 1, background: '#f8f9fc', borderRadius: '12px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, color: '#2b3674', margin: 0 }}>Define Wing</h2>
        <button 
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded w-[90%] max-w-2xl shadow-lg flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center px-4 py-3 border-b border-gray-200">
              <h3 className="text-gray-600 font-medium text-lg">{editItem ? 'Edit Define Wing' : 'Add New Define Wing'}</h3>
