    ...
    <div style={{ flex: 1, background: '#f8f9fc', padding: '32px', minHeight: '100vh', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 28 }}>
        {loading ? (
          ...
          <p style={{ color: '#64748b' }}>No videos available yet.</p>
        ) : videos.map(video => (
          <div 
            key={video._id} 
            style={{ 
          ...
          >
            {/* Thumbnail Area */}
            <div style={{ position: 'relative', height: 200, backgroundImage: `url(${video.thumbnail || 'https://via.placeholder.com/400x200?text=No+Thumbnail'})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
            ...
            <div style={{ padding: '20px 24px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#64748b', fontSize: 13, fontWeight: 600 }}>
                <FaCalendarAlt style={{ color: '#94a3b8' }} /> {video.eventDate ? new Date(video.eventDate).toLocaleDateString() : video.date || ''}
              </div>
            </div>
          </div>
        ))}
