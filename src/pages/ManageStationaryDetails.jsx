...
export default function ManageStationaryDetails() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('http://localhost:5005/api/stationary-details');
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        await fetch(`http://localhost:5005/api/stationary-details/${editItem._id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: inputValue })
        });
      } else {
        await fetch('http://localhost:5005/api/stationary-details', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: inputValue })
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      try {
        await fetch(`http://localhost:5005/api/stationary-details/${id}`, { method: 'DELETE' });
        fetchData();
      } catch (err) {
        console.error(err);
      }
