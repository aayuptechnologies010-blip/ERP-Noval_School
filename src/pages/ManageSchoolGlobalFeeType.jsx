...
const schoolFeeTypeFields = [
  ...
  { label: 'Contact No.', key: 'contactNo', type: 'tel', pattern: '[0-9]*', title: 'Please enter numbers only' },
...
function ManageSchoolGlobalFeeType() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/school-global-fee-types`, {
  ...
  const handleSave = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = editItem 
        ? `${import.meta.env.VITE_API_BASE_URL}/api/school-global-fee-types/${editItem._id}`
        : `${import.meta.env.VITE_API_BASE_URL}/api/school-global-fee-types`;
  ...
  const openAddModal = () => {
    setModalInput({ isAdmin: false, feeType: 'School Fee' });
    setEditItem(null);
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this fee type detail?')) return;
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/school-global-fee-types/${id}`, {
  ...
  return (
    <div style={{ flex: 1, background: '#f8f9fc', borderRadius: '12px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <div style={{ padding: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, color: '#2b3674', margin: 0 }}>School Global Details With FeeType</h2>
        <button 
            ...
            {items.length === 0 ? (
              ...
              items.map((row, index) => (
                <tr key={row._id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3">{row.feeType}</td>
                  <td className="px-6 py-3">{row.isAdmin ? 'True' : 'False'}</td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded w-[90%] max-w-5xl shadow-lg flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center px-4 py-3 border-b border-gray-200">
              <h3 className="text-gray-600 font-medium text-lg">{editItem ? 'Edit FeeType Details' : 'Add New FeeType Detail'}</h3>
                ...
                <div className="grid grid-cols-4 gap-x-6 gap-y-4">
                  ...
                  <div className="flex flex-col gap-1 justify-end pb-1.5">
                    <label className="flex items-center gap-2 cursor-pointer text-sm font-bold text-gray-700">
                      <input 
                        type="checkbox" 
                        checked={modalInput.isAdmin || false}
                        onChange={(e) => setModalInput({...modalInput, isAdmin: e.target.checked})}
      ...
      {viewItem && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded w-[90%] max-w-5xl shadow-lg flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center px-4 py-3 border-b border-gray-200">
              <h3 className="text-gray-600 font-medium text-lg">View FeeType Details</h3>
            ...
            <div className="p-6 overflow-y-auto">
              <div className="grid grid-cols-4 gap-x-6 gap-y-4">
                ...
                <div className="flex flex-col gap-1">
                  <label className="text-xs font-bold text-gray-500">Is Admin</label>
                  <div className="text-sm text-gray-800 font-medium">{viewItem.isAdmin ? 'True' : 'False'}</div>
                </div>
              </div>
            </div>
            <div className="p-4 flex justify-center border-t border-gray-100">
...
export default ManageSchoolGlobalFeeType;
