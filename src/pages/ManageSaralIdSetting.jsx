...
export default function ManageSaralIdSetting() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch('http://localhost:5005/api/saral-id-settings');
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        await fetch(`http://localhost:5005/api/saral-id-settings/${editItem._id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: inputValue })
        });
      } else {
        await fetch('http://localhost:5005/api/saral-id-settings', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: inputValue })
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this item?")) {
      try {
        await fetch(`http://localhost:5005/api/saral-id-settings/${id}`, { method: 'DELETE' });
        fetchData();
      } catch (err) {
        console.error(err);
      }
  ...
  return (
    <div className="bg-white p-6 mt-4 flex flex-col items-end w-full">
      <div className="mb-4 w-full flex justify-between items-center">
        <h2 className="text-xl font-bold text-gray-700">Saral ID Setting</h2>
        ...
        <table className="w-full border-collapse border border-gray-300 text-sm">
          <thead>
            <tr className="bg-[#32a3d7] text-white">
              ...
              <th className="px-6 py-3 border border-gray-300 font-medium text-left">Saral ID Setting Name <FaAngleUp className="inline text-[10px]" /></th>
